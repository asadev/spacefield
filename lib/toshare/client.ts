...
/* Client-side helper for tools to mint a toshare.net link.
 *
 * Tools call mintShareLink() from the browser. Behind the scenes it
 * server-actions to lib/toshare/server.ts and returns the public URL.
 */

import type { ToShareType } from "./types";
...
export async function mintShareLink(input: MintShareLinkInput): Promise<MintShareLinkResult> {
  try {
    const res = await fetch("/api/toshare/mint", {
      method: "POST",
