 ...
 *     exception for toShare viewer paths and `/embed/*` which exist
...
/* SE-008 — toShare viewer prefixes + the generic `/embed` namespace
 ...
 * - `/p/*` `/q/*` `/r/*` `/b/*` `/d/*` — toShare viewers (Proposal /
 *   Quote / Report / Brochure / Document, see lib/toShare).
 */
 ...
 * paths (toShare viewers + `/embed/*`). Callers that don't have a
...
export function applySecurityHeaders(
  ...
  if (!embed) {
    ...
    // toShare viewers + `/embed/*` can render on customer sites.
    response.headers.set(
      "Content-Security-Policy-Report-Only",
