...
/**
 * Redirect links bounce visitors from the trusted share.example.com/<slug> origin
 * to an external destination. Without validation this is an open-redirect /
 * phishing relay (SHARE-01): any signed-in user could mint a link whose
 ...
 * mint time (lib/share/server.ts) and view time (app/(share)/r/[slug]).
