-- Security hardening for the toShare RPCs (Fix Agent V-3, SA-001).
--
-- 1. `toshare_mint` previously trusted whatever workspace_id the client
...
--
-- 2. `toshare_record_submit` accepts a `jsonb` payload from anon callers
--    on the public form viewer. With no size check, a single attacker
--    can pump megabyte-sized JSON blobs into `toshare_events.payload`
