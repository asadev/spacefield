-- ─────────────────────────────────────────────────────────────────────────
-- toShare booking event RPC + booked-slot lookup helper.
--
-- Booking flow: visitor picks a slot → POST to /api/toshare/book →
-- toshare_record_booking RPC inserts a 'submit' event with the chosen
-- slot, and bumps submit_count. Already-booked slots are then visible to
-- new visitors so they can't double-book.
-- ─────────────────────────────────────────────────────────────────────────

create or replace function public.toshare_record_booking(
  p_link_id     uuid,
  p_start_local text,
  p_invitee_name text,
...
create or replace function public.toshare_booked_slots(p_link_id uuid)
returns setof text
language sql
security definer
set search_path = public
...
grant execute on function public.toshare_record_booking to anon, authenticated;
grant execute on function public.toshare_booked_slots   to anon, authenticated;
