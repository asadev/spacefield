-- ─────────────────────────────────────────────────────────────────────────
-- Share link system — universal mint/host/track for any tool's output.
--
-- Domain: share.example.com (separate from spacefield.io for reputation isolation).
-- Patterns:
--   - Free  : share.example.com/<type>/<slug>           (e.g. /f/abc123, /p/xyz)
--   - Paid  : <workspace>.share.example.com/<slug>      (custom subdomain)
--   - Vanity: share.example.com/<type>/<custom-slug>    (paid only)
--
-- Types (`link_type`):
...
create index if not exists share_links_workspace_idx on public.share_links (workspace_id, created_at desc);
create index if not exists share_links_owner_idx     on public.share_links (owner_user_id, created_at desc);
create index if not exists share_links_subdomain_idx on public.share_links (custom_subdomain) where custom_subdomain is not null;
create index if not exists share_links_active_idx    on public.share_links (slug) where status = 'active';
...
create index if not exists share_events_link_idx on public.share_events (link_id, created_at desc);
...
create or replace function public.share_record_view(
  p_slug      text,
  p_subdomain text,
  p_ip_hash   text,
  p_ua_hash   text,
...
begin
  ...
  if v_link_id is not null then
    insert into public.share_events (link_id, event, ip_hash, ua_hash, referrer)
...
create policy "share_links_read_own"
...
create policy "share_events_read_own"
...
grant execute on function public.share_record_view    to anon, authenticated;
grant execute on function public.share_record_submit  to anon, authenticated;

comment on table public.share_links is 'Universal link/share registry served from share.example.com. Each row backs one public URL minted by some tool.';
comment on table public.share_events is 'Per-link view/submit/convert events for analytics. Append-only.';
