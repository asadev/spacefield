-- ─────────────────────────────────────────────────────────────────────────
-- toShare quote-accept RPC
--
-- toshare_events has RLS — direct inserts from anon/authenticated are blocked
-- by design. This SECURITY DEFINER function records a 'convert' event AND
-- bumps the link's submit_count atomically, just like toshare_record_submit
-- does for forms.
-- ─────────────────────────────────────────────────────────────────────────

create or replace function public.toshare_record_accept(
  p_link_id      uuid,
  p_signer_name  text,
  p_signer_email text,
...
grant execute on function public.toshare_record_accept to anon, authenticated;

comment on function public.toshare_record_accept is
  'Records a quote acceptance — used by /api/toshare/accept';
