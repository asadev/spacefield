...
create table if not exists public.workspace_custom_domains (
  ...
  scope              text not null default 'workspace'
                       check (scope in ('workspace','share')),
...
-- 12. ───────────────── Share analytics RPCs ─────────────────
-- Per-link daily time series + funnel.
create or replace function public.share_link_analytics(
  p_link_id uuid,
...
as $$
  with src as (
    select * from public.share_events
...
grant execute on function public.share_link_analytics(uuid, timestamptz) to authenticated;

-- Top links + per-type totals for a workspace (for the analytics dashboard).
create or replace function public.share_workspace_analytics(
  p_workspace_id uuid,
  p_since timestamptz default (now() - interval '30 days')
)
returns table (
...
as $$
  select
    ...
    coalesce((select count(*)::int from public.share_events e
    ...
    case
      when coalesce((select count(*) from public.share_events e
                      where e.link_id = l.id and e.event = 'view' and e.created_at >= p_since), 0) > 0
      then round(
        100.0 * coalesce((select count(*) from public.share_events e
                           where e.link_id = l.id and e.event in ('submit','convert')
                             and e.created_at >= p_since), 0)::numeric
        / nullif((select count(*) from public.share_events e
                   where e.link_id = l.id and e.event = 'view' and e.created_at >= p_since), 0)::numeric,
        2)
      else 0::numeric
    end as conv_rate,
    (select max(e.created_at) from public.share_events e where e.link_id = l.id) as last_event
...
grant execute on function public.share_workspace_analytics(uuid, timestamptz) to authenticated;

-- Cross-workspace totals for super-admin. Uses admin_caller_is_admin
-- so RLS doesn't matter — the function refuses non-admins.
create or replace function public.share_global_analytics(
  p_since timestamptz default (now() - interval '30 days')
)
returns jsonb
language plpgsql
...
begin
  ...
  select jsonb_build_object(
    'total_links',     (select count(*)::int from public.share_links),
    'active_links',    (select count(*)::int from public.share_links where status = 'live'),
    'total_views',     (select count(*)::int from public.share_events where event = 'view' and created_at >= p_since),
    'total_submits',   (select count(*)::int from public.share_events where event = 'submit' and created_at >= p_since),
    'total_converts',  (select count(*)::int from public.share_events where event = 'convert' and created_at >= p_since),
    'by_type',         (select coalesce(jsonb_object_agg(type, c), '{}'::jsonb)
                          from (select type, count(*)::int as c
                                  from public.share_links group by type) t),
    'webhook_success_rate', coalesce(
        (select round(100.0 * count(*) filter (where status = 'success')::numeric
                       / nullif(count(*), 0)::numeric, 2)
            from public.share_webhook_deliveries
...
grant execute on function public.share_global_analytics(timestamptz) to authenticated;
...
values
  ('share.paid_domains', 'Share paid custom domains', 'Allow workspaces to attach a custom CNAMEd domain (e.g. share.example.com).', false, 'off'),
