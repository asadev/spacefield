...
function flagEmoji(code: string | null | undefined): string {
  if (!code || code.length !== 2) return "🏳"; // white flag fallback
...
// ---------------------------------------------------------------------------
// City metric rows — map CityData fields to display rows organized by category
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Qualitative value (supply pipeline / yes-no-limited)
// ---------------------------------------------------------------------------
...
// In-tool hero header
// ---------------------------------------------------------------------------
function ToolHero({ frame }: { frame: boolean }) {
  return (
      ...
      <div className="relative flex flex-col gap-4 px-5 py-5 md:flex-row md:items-end md:justify-between md:px-6 md:py-6">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full border border-tool-accent/30 bg-tool-accent-soft px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-tool-accent">
              USD normalised
            </span>
          </div>
          ...
          <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-secondary">
            Compare real estate markets worldwide. 48 curated cities plus live country indices from BIS, OECD &amp; World Bank — all values converted to USD for apples-to-apples reading.
          </p>
        </div>
      </div>
    </div>
...
// Page wrappers
// ---------------------------------------------------------------------------
export default function GlobalMarketComparisonPage() {
  return (
...
function GlobalMarketComparison() {
        ...
        <section
          className={`relative mx-auto ${
            frame
              ...
              : "max-w-[1400px] px-4 pb-[clamp(80px,12vw,160px)] sm:px-6 lg:px-10"
          }`}
        >
          <motion.div
            ...
            className={`overflow-hidden border border-app bg-app-elevated ring-1 ring-tool-accent/10 ${
              frame ? "rounded-lg" : "mt-6 rounded-xl"
            }`}
          >
            <ToolHero frame={frame} />
                ...
                <>
                  ...
                  {mode === "cities" && (
                    <>
                      ...
                      {/* Category metric filter (pill row) */}
                      <div className="mt-5">
                        <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-tool-accent">
                          Metric Category
                        </h3>
                                    ...
                                    >
                                      <td className="sticky left-0 z-10 bg-inherit px-4 py-3 border-r border-app">
                                        <div className="flex flex-col gap-0.5">
                                          ...
                                          <span className="hidden text-[9px] text-muted leading-snug sm:block">
                                            {metric.description}
                                          </span>
                                          <span className="hidden text-[9px] uppercase tracking-wider text-tool-accent sm:block">
                                            {metric.source}
                                          </span>
                                        </div>
                                      </td>
          ...
          {!frame && <ToolRecommendations slug="global-market-comparison" />}
        </section>
      </div>
    </AuthGate>
  );
