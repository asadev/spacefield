...
function calculateScreeningScore(area: AreaRentalProfile): number {
  ...
  const riskNorm = area.riskLevel === "low" ? 100 : area.riskLevel === "medium" ? 50 : 10;
...
function getRentForType(area: AreaRentalProfile, type: PropertyType): number {
  switch (type) {
    case "Studio": return area.avgRentStudio;
    case "1BR": return area.avgRent1BR;
    case "2BR": return area.avgRent2BR;
    case "3BR": return area.avgRent3BR;
  }
}
...
function DemandDial({ score }: { score: number }) {
...
/* ───── Stacked Score Breakdown (per-category bars in tool-accent variants) ───── */
...
export default function TenantScreeningPage() {
  const [selectedArea, setSelectedArea] = useState<string>(AREA_RENTAL_DATA[0]?.area || "");
  ...
  const rankedAreas = useMemo(() => {
    ...
    scored.sort((a, b) => {
      switch (sortKey) {
        case "score": return sortAsc ? a.score - b.score : b.score - a.score;
        case "area": return sortAsc ? a.area.localeCompare(b.area) : b.area.localeCompare(a.area);
        case "demandScore": return sortAsc ? a.demandScore - b.demandScore : b.demandScore - a.demandScore;
        case "vacancyRate": return sortAsc ? a.vacancyRate - b.vacancyRate : b.vacancyRate - a.vacancyRate;
        case "riskLevel": {
  ...
  const pricingAnalysis = useMemo(() => {
    ...
    return { rent, marketRent, diff, position, suggestedLow, suggestedHigh, baseVacancy, estimatedVacancy };
  ...
  const generateReport = useCallback(() => {
    ...
    report += `  YoY Change: ${areaData.yoyRentChange > 0 ? "+" : ""}${areaData.yoyRentChange}%\n`;
  ...
  // Verdict accent — Approve uses tool-accent (amber), Review uses amber-500/40 ramp, Decline uses rose
  ...
  const riskChipClass = (level: string) => {
    switch (level) {
      case "low": return "bg-tool-accent-soft text-tool-accent border-tool-accent/30";
      case "medium": return "bg-amber-500/10 text-amber-600 dark:text-amber-300 border-amber-500/30";
      case "high": return "bg-rose-500/10 text-rose-600 dark:text-rose-300 border-rose-500/30";
      default: return "";
    }
  };
  ...
  const referenceContacts = [
    { role: "Current Employer / HR", purpose: "Verify employment, tenure, salary band." },
    { role: "Previous Landlord", purpose: "Confirm payment history and conduct." },
    { role: "Bank Reference", purpose: "Cross-check account standing and balance." },
  ];
    ...
    <AuthGate>
      ...
      >
        ...
        <div className="tool-hero border-b border-app">
          <div className="mx-auto max-w-[1280px] px-4 sm:px-6 pt-8 pb-6 lg:px-10">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-tool-accent/30 bg-tool-accent-soft">
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 24 24"
                  ...
                  <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl text-app">
                    Tenant Screening
                  </h1>
                </div>
                <p className="mt-2 max-w-xl text-sm text-secondary">
                  Demand scoring across {AREA_RENTAL_DATA.length} areas. Vacancy risk, optimal rent band, and
                  a verdict you can defend.
                </p>
              </div>
              <div className="inline-flex items-center gap-2 rounded-full border border-tool-accent/30 bg-tool-accent-soft px-3 py-1.5 text-[0.65rem] uppercase tracking-[0.2em] text-tool-accent">
        ...
        <section className="pb-[clamp(80px,12vw,160px)]">
          <div className="mx-auto max-w-[1280px] px-4 sm:px-6 pt-6 lg:px-10 space-y-5">

            {/* ───── Input strip ───── */}
            <div className="rounded-xl border border-app bg-app-elevated p-5 transition-colors">
              <div className="grid grid-cols-1 md:grid-cols-[1.4fr_2fr_1.2fr] gap-4">
                <div>
                  <label className="block text-[10px] uppercase tracking-[0.2em] text-muted font-medium mb-1.5">
                    Area
                  </label>
                  <select
                    value={selectedArea}
                    onChange={(e) => setSelectedArea(e.target.value)}
                    className="w-full rounded-lg border border-app bg-app px-3 py-2.5 text-sm text-app transition-colors focus:outline-none focus:border-tool-accent focus:ring-2 focus:ring-tool-accent/20"
                  >
                ...
                <div>
                  ...
                  <div className="flex gap-1 rounded-lg border border-app bg-app p-1">
                    {(["Studio", "1BR", "2BR", "3BR"] as PropertyType[]).map((t) => (
                      <button
                        key={t}
                        type="button"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-app bg-app px-3 py-2.5 text-sm text-app placeholder:text-muted transition-colors focus:outline-none focus:border-tool-accent focus:ring-2 focus:ring-tool-accent/20"
                  />
                </div>
              </div>
            </div>
              ...
              {areaData && factors ? (
                ...
                >
                  ...
                  <motion.div
                    ...
                    className={`relative overflow-hidden rounded-2xl border ${tone.banner} p-6 sm:p-8`}
                  >
                    <div aria-hidden className="absolute inset-0 tool-hero opacity-50 pointer-events-none" />
                    <div className="relative grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] gap-6 items-center">
                      {/* Verdict + dial */}
                      <div className="flex flex-col items-center text-center">
                        <div className="inline-flex items-center gap-2 mb-3">
                          <span className={`h-2 w-2 rounded-full ${tone.dot} animate-pulse`} />
                          <span className={`text-[10px] uppercase tracking-[0.25em] font-semibold ${tone.text}`}>
                            Verdict
                          </span>
                        </div>
                        <div className={`text-5xl sm:text-6xl font-bold tracking-tight ${tone.text}`}>
                          {v.verdict}
                        </div>
                      ...
                      <div className="relative flex flex-col items-center">
                        <DemandDial score={screeningScore} />
                        <div className="absolute top-[72px] text-center pointer-events-none">
                          <div className="text-5xl font-bold tabular-nums text-tool-accent leading-none">
                            {screeningScore}
                          </div>
                        ...
                        <div className="mt-3 flex flex-wrap items-center justify-center gap-2 text-xs">
                          ...
                          <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider border rounded-full ${riskChipClass(areaData.riskLevel)}`}>
                            {areaData.riskLevel} risk
                          </span>
                          <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider border rounded-full bg-app-elevated text-secondary border-app`}>
                            {areaData.segment}
                          </span>
                        </div>
                      </div>
                  ...
                  {/* ───── Score Breakdown — stacked bars in tool-accent variants ───── */}
                  <div className="rounded-xl border border-app bg-app-elevated p-5 transition-colors">
                    <div className="mb-4 flex items-center justify-between">
                      <p className="text-[0.6rem] uppercase tracking-[0.25em] text-muted font-medium">
                        Score Breakdown · Bureau Signals
                      </p>
                      <span className="text-[0.55rem] uppercase tracking-[0.2em] text-tool-accent font-medium">
                        ● 5 categories
                      </span>
                    </div>
                    <div className="space-y-3">
                      <CategoryBar label="Demand" value={factors.demand} weight={SCORING_WEIGHTS.demandScore} />
                      <CategoryBar label="Occupancy" value={factors.vacancy} weight={SCORING_WEIGHTS.vacancyRate} />
                      <CategoryBar label="Leasing Speed" value={factors.leasingSpeed} weight={SCORING_WEIGHTS.daysToLease} />
                      <CategoryBar label="Rent Growth" value={factors.rentGrowth} weight={SCORING_WEIGHTS.yoyRentChange} />
                      <CategoryBar label="Retention" value={factors.retention} weight={SCORING_WEIGHTS.renewalRate} />
                    </div>
                    ...
                    {suggestedRange && (
                      <div className="pt-5 mt-5 border-t border-app">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs font-semibold text-tool-accent tabular-nums">
                            {formatAED(suggestedRange.low)} – {formatAED(suggestedRange.high)}
                          </span>
                        </div>
                        <div className="relative h-8 rounded-md border border-app overflow-hidden bg-app">
                          <div className="absolute inset-0 bg-gradient-to-r from-tool-accent/15 via-tool-accent-soft to-tool-accent/30" />
                          <div className="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 w-0.5 bg-tool-accent" />
                          <div className="absolute top-1 left-1/2 -translate-x-1/2 text-[9px] font-semibold text-tool-accent uppercase tracking-wider">
                          ...
                          {pricingAnalysis && (() => {
                            ...
                            return (
                              ...
                              >
                                <div className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] font-semibold whitespace-nowrap text-app">
                                  you
                                </div>
                              </div>
                            );
                        ...
                        <div className="flex justify-between mt-1.5 text-[10px] text-muted uppercase tracking-wider">
                          <span>fill fast</span>
                        ...
                        {pricingAnalysis && (
                          <div className="mt-3 text-xs text-secondary">
                            Likely <span className="font-semibold text-app tabular-nums">~{pricingAnalysis.estimatedVacancy} days</span> vacancy at {formatAED(pricingAnalysis.rent)}.
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  {/* ───── Document checklist + Reference contact tiles ───── */}
                  <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] gap-5">
                    ...
                    <div className="rounded-xl border border-app bg-app-elevated p-5 transition-colors">
                      <div className="mb-4 flex items-center justify-between">
                        ...
                        <span className="text-[0.55rem] uppercase tracking-[0.2em] text-tool-accent font-medium">
                          ● {Object.values(docChecked).filter(Boolean).length}/{documentChecklist.length} collected
                        </span>
                      </div>
                            ...
                            >
                              <span
                                className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded border transition-colors ${
                                  checked
                                    ? "bg-tool-accent border-tool-accent"
                                    : "border-app bg-app"
                                }`}
                              >
                                {checked && (
                              ...
                              <span className={`text-sm ${checked ? "text-app" : "text-secondary"}`}>
                                {doc.label}
                              </span>
                            </button>
                          );
                          ...
                          >
                            <div className="flex items-start gap-2">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="text-sm font-semibold text-app">{ref.role}</div>
                                <div className="text-xs text-muted leading-relaxed mt-0.5">{ref.purpose}</div>
                              </div>
                            </div>
                          </div>
                        ))}
                      </div>
                      <div className="mt-4 pt-4 border-t border-app flex items-center justify-between text-xs">
                        <span className="text-muted">Renewal Rate (area avg)</span>
                        <span className="font-semibold text-tool-accent tabular-nums">{areaData.renewalRate}%</span>
                      </div>
                    </div>
                  </div>
                      ...
                      {topByDemand.map((a, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full grid grid-cols-[24px_minmax(0,1.3fr)_minmax(0,2fr)_90px_minmax(0,1fr)] items-center gap-4 px-5 py-3 text-left transition-colors ${
                          ...
                          >
                            ...
                            <span className={`text-sm font-medium truncate ${isSel ? "text-tool-accent" : "text-app"}`}>
                              {a.area}
                            </span>
                            <div className="flex items-center gap-2">
                              <div className="relative h-2 w-full rounded-full bg-app overflow-hidden border border-app">
                                <div
                                  className="h-full rounded-full bg-tool-accent"
                                  style={{ width: `${a.demandScore}%`, opacity: isSel ? 1 : 0.7 }}
                                />
                              </div>
                              <span className="text-xs font-semibold tabular-nums text-secondary w-6 text-right">
                                {a.demandScore}
                              </span>
                            </div>
                            <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider border rounded-full text-center ${riskChipClass(a.riskLevel)}`}>
                              {a.riskLevel}
                            </span>
                            <span className="text-xs text-secondary text-right tabular-nums">
                              {formatAED(rent)}
                            </span>
                          </button>
                        );
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">
                        {(["Studio", "1BR", "2BR", "3BR"] as PropertyType[]).map((t) => (
                          ...
                          >
                            ...
                            <div className="text-sm font-semibold text-app tabular-nums">
                              {formatAED(getRentForType(areaData, t))}
                            </div>
                          </div>
                        ))}
                      </div>
                      <div className="flex items-center justify-between py-2 border-t border-app">
                        <span className="text-xs text-muted">YoY Rent Change</span>
                        <span className={`text-xs font-semibold tabular-nums ${areaData.yoyRentChange > 0 ? "text-tool-accent" : "text-rose-500"}`}>
                          {areaData.yoyRentChange > 0 ? "+" : ""}{areaData.yoyRentChange}%
                        </span>
                      </div>
                      <div className="flex items-center justify-between py-2 border-t border-app">
                        <span className="text-xs text-muted">Avg Days to Lease</span>
                        <span className="text-xs font-semibold text-app tabular-nums">
                    ...
                    <div className="rounded-xl border border-app bg-app-elevated p-5 transition-colors">
                      ...
                      <div className="space-y-3 text-sm">
                        <div>
                          <div className="text-[10px] uppercase tracking-wider text-muted mb-1">Typical</div>
                          <div className="text-app">{areaData.tenantDemographic}</div>
                        </div>
                        <div>
                          <div className="text-[10px] uppercase tracking-wider text-muted mb-1.5">Top Nationalities</div>
                          <div className="flex flex-wrap gap-1.5">
                            {areaData.topNationalities.map((n) => (
                              <span
                                key={n}
                                className="px-2 py-0.5 text-[11px] bg-app rounded-md text-secondary border border-app"
                              >
                                {n}
                              </span>
                            ))}
                        ...
                        <div>
                          <div className="text-[10px] uppercase tracking-wider text-muted mb-1.5">Insights</div>
                          ...
                          <tr className="border-b border-app bg-app">
                            {[
                              ...
                              <th
                                ...
                                className="text-left py-2.5 px-4 text-[10px] uppercase tracking-[0.25em] text-muted font-medium cursor-pointer hover:text-tool-accent transition-colors"
                              >
                                {col.label}
                                {sortKey === col.key && (
                                  <span className="ml-1 text-tool-accent">{sortAsc ? "↑" : "↓"}</span>
                                )}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {rankedAreas.map((area) => (
                            <tr
                              ...
                              className={`border-b border-app/60 cursor-pointer transition-colors ${
                                area.area === selectedArea
                                  ? "bg-tool-accent-soft"
                                  : "hover:bg-app"
                              }`}
                            >
                              <td className="py-2.5 px-4 text-app font-medium">{area.area}</td>
                              <td className="py-2.5 px-4 font-semibold tabular-nums text-tool-accent">
                                {area.score}
                              </td>
                              <td className="py-2.5 px-4 text-secondary tabular-nums">{area.demandScore}</td>
                              <td className="py-2.5 px-4 text-secondary tabular-nums">{area.vacancyRate}%</td>
                              <td className="py-2.5 px-4">
                                <span className={`px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider border rounded-full ${riskChipClass(area.riskLevel)}`}>
                                  {area.riskLevel}
                                </span>
                              </td>
                            </tr>
                  ...
                  <div className="rounded-xl border border-app bg-app-elevated p-5 transition-colors">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          className="px-4 py-2 rounded-lg border border-tool-accent bg-tool-accent text-white text-xs font-semibold uppercase tracking-[0.15em] transition-opacity hover:opacity-90"
                        >
                          Approve Tenant
                        </button>
                        <button
                          type="button"
                          className="px-4 py-2 rounded-lg border border-app bg-app-elevated text-app text-xs font-semibold uppercase tracking-[0.15em] transition-colors hover:bg-app"
                        >
                          Reject
                        </button>
                        <span className="hidden sm:block w-px self-stretch bg-app mx-1" />
                        <button
                          type="button"
                          onClick={copyReport}
                          className="px-4 py-2 rounded-lg border border-app bg-app-elevated text-app text-xs font-semibold uppercase tracking-[0.15em] transition-colors hover:bg-app"
                ...
                >
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-tool-accent-soft border border-tool-accent/30 rounded-full text-xs font-semibold text-tool-accent uppercase tracking-wider mb-3">
                    Awaiting input
                  </div>
            ...
            <div className="pt-10">
              <ToolRecommendations slug="tenant-screening" />
            </div>
          </div>
        </section>
      </div>
