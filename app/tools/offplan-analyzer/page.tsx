...
// ToolShell + ScenarioBar imports removed — those modules don't exist in
// this codebase. The page renders bare; the desktop window provides the shell.
...
function OffplanAnalyzerInner() {
  ...
  const body = (
    <div data-tool-theme="investment" data-tool="offplan-analyzer" className="bg-app text-app">
          ...
          >
            ...
            <div className="relative grid gap-6 lg:grid-cols-[1.4fr_1fr] items-center">
              ...
              <div>
                ...
                <p className={`font-mono text-5xl sm:text-6xl font-bold tabular-nums tracking-tight ${npvIsPositive ? "text-tool-accent" : "text-rose-500 dark:text-rose-400"}`}>
      ...
      <section className="px-4 sm:px-6 pt-5 pb-2">
        <form onSubmit={handleCalculate}>
          <div className={`grid gap-5 ${plans.length > 1 ? "lg:grid-cols-2 xl:grid-cols-3" : "max-w-2xl"}`}>
            {plans.map((plan, pi) => {
      ...
      {!frameMode && (
        <div className="px-4 sm:px-6 mt-8 pb-12">
          <ToolRecommendations slug="offplan-analyzer" />
        </div>
      )}
    </div>
  );

  return <AuthGate>{body}</AuthGate>;
}
