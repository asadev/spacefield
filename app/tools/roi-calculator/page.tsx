...
function ScenarioForm({
    ...
    <div className="flex items-center border-b border-app py-2.5 last:border-b-0">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="w-28 border border-app bg-app-elevated px-2 py-1 text-right font-mono text-[0.82rem] tabular-nums text-app outline-none transition-colors rounded focus:border-tool-accent focus:ring-1 focus:ring-tool-accent placeholder:text-muted"
        />
  ...
  return (
    <motion.div
      ...
      className="bg-app-elevated border border-app rounded-xl overflow-hidden"
    >
      <div className="flex items-center justify-between bg-tool-accent-soft border-b border-app px-4 py-2.5">
        <div className="flex items-center gap-2">
      ...
      <div className="p-4 space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-1 p-1 bg-app rounded-lg border border-app">
            {(["cash", "mortgage"] as const).map((mode) => (
              <button
                key={mode}
                type="button"
                onClick={() => onUpdate(scenario.id, "mode", mode)}
                className={`rounded-md px-3 py-1.5 text-[0.68rem] uppercase tracking-[0.14em] font-medium transition-all duration-200 ${
                  scenario.mode === mode
...
function ROIBarChart({
  ...
  useEffect(() => {
    const draw = () => {
      ...
      const bars = [
        { label: "Purchase", value: purchasePrice, color: "rgba(59,130,246,0.75)" },
        { label: "Invested", value: totalInvestment, color: "rgba(37,99,235,0.85)" },
        { label: "End Value", value: endValue, color: "rgba(29,78,216,0.95)" },
      ];
...
function VerdictChip({ roi }: { roi: number }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[0.62rem] uppercase tracking-[0.18em] font-semibold ${cls}`}>
...
function ROICalculatorInner() {
  ...
  return (
    <AuthGate>
      ...
      >
        {/* Hero glow */}
        <div aria-hidden className="absolute inset-x-0 top-0 h-[420px] tool-hero opacity-60 pointer-events-none" />
        <div
          aria-hidden
        ...
        <main className="relative z-10">
          {/* Hero */}
          {!isFramed && (
            <header className="mx-auto max-w-[1300px] px-6 lg:px-10 pt-10 pb-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted">/ roi-engine</span>
              </div>
              <div className="flex items-end justify-between gap-6 flex-wrap">
                <div>
                  <h1 className="text-3xl lg:text-4xl font-semibold tracking-tight flex items-baseline gap-3">
                    ROI Calculator
                    <span className="h-[3px] w-16 rounded-full bg-tool-accent" />
                  </h1>
                  <p className="mt-2 text-sm text-secondary max-w-2xl">
                    Cap rate, cash-on-cash, IRR. Year-by-year. One scenario or two side-by-side.
                  </p>
                </div>
                <div className="bg-app-elevated border border-app rounded-xl px-4 py-2.5 text-right">
                  <p className="text-[0.58rem] uppercase tracking-[0.2em] text-muted">Horizon</p>
                  <p className="font-mono text-xl font-semibold tabular-nums text-tool-accent">
                    {horizonYears}
                    <span className="text-sm text-muted ml-1">yrs</span>
                  </p>
                </div>
              </div>
            </header>
          ...
          <section className="pb-[clamp(80px,12vw,160px)]">
            <div className={`mx-auto max-w-[1300px] px-6 lg:px-10 ${isFramed ? "pt-6" : "pt-4"}`}>
              <form onSubmit={handleCalculate}>
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
                  {/* LEFT: inputs */}
                  <div className="space-y-5">
                    <div
                      className={`grid gap-4 ${scenarios.length > 1 ? "md:grid-cols-2" : "grid-cols-1"}`}
                    >
                      {scenarios.map((sc, i) => (
                        <ScenarioForm
                          key={sc.id}
                    ...
                    <div className="bg-app-elevated border border-app rounded-xl p-4">
                      ...
                      <div className="grid grid-cols-4 gap-1 p-1 bg-app rounded-lg border border-app">
                      ...
                      <div className="mt-3 flex gap-2">
                        {scenarios.length < 2 && (
                          <button
                            type="button"
                            onClick={addScenario}
                            className="flex-1 border border-dashed border-app bg-app-elevated px-4 py-2.5 text-[0.68rem] uppercase tracking-[0.14em] text-tool-accent font-medium transition-all duration-200 hover:bg-tool-accent-soft rounded-md"
                          >
                            + Add Scenario
                          </button>
                        )}
                        <button
                          type="submit"
                          className="flex-1 bg-tool-accent px-5 py-2.5 text-[0.7rem] uppercase tracking-[0.14em] text-white font-semibold transition-all duration-200 hover:brightness-110 rounded-md shadow-sm"
                        >
                          Calculate ROI →
                        </button>
                      </div>
                  ...
                  <div className="lg:sticky lg:top-6 self-start">
                    {!showResults || results.length === 0 || !activeResult ? (
                      <div className="bg-app-elevated border border-app rounded-xl overflow-hidden">
                        <div className="bg-tool-accent-soft border-b border-app px-4 py-2.5">
                          <p className="text-[0.62rem] font-semibold uppercase tracking-[0.2em] text-tool-accent">
                            Results — Pending
                          </p>
                        </div>
                        <div className="p-10 text-center">
                          <div className="font-mono text-[0.7rem] text-muted tabular-nums leading-relaxed">
                            <div>— — — — — — — — — — —</div>
                      ...
                      <div ref={resultsRef} className="space-y-4">
                        ...
                        {results.length > 1 && (
                          <div className="flex items-center justify-between gap-2">
                            <div className="grid grid-cols-2 gap-1 p-1 bg-app-elevated border border-app rounded-lg flex-1">
                              {results.map((r, ri) => (
                                <button
                                  key={ri}
                                  type="button"
                                  onClick={() => setActiveScenarioId(scenarios[ri].id)}
                                  className={`rounded-md px-3 py-1.5 text-[0.7rem] uppercase tracking-[0.14em] font-medium transition-all duration-200 ${
                                    activeIndex === ri
                                      ? "bg-tool-accent text-white shadow-sm"
                                      : "text-secondary hover:text-tool-accent"
                                  }`}
                        ...
                        <motion.div
                          ...
                          className="bg-app-elevated border border-app rounded-xl overflow-hidden"
                        >
                          <div className="bg-tool-accent-soft border-b border-app px-4 py-2.5 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className="h-2 w-2 rounded-full bg-tool-accent" />
                              <p className="text-[0.62rem] font-semibold uppercase tracking-[0.2em] text-tool-accent">
                          ...
                          <div className="px-6 py-7 text-center">
                            <p className="text-[0.6rem] uppercase tracking-[0.25em] text-muted mb-2 font-semibold">
                            ...
                            <div className="mt-5 flex items-center justify-center gap-6 text-[0.7rem]">
                              <div>
                                <p className="text-muted uppercase tracking-[0.15em] text-[0.55rem] font-semibold">Down Payment</p>
                                <p className="font-mono tabular-nums text-secondary mt-0.5">
                                  AED {fmt(activeResult.downPayment)}
                                </p>
                              </div>
                              <span className="h-6 w-px bg-app" />
                              <div>
                                <p className="text-muted uppercase tracking-[0.15em] text-[0.55rem] font-semibold">End Value</p>
                                <p className="font-mono tabular-nums text-secondary mt-0.5">
                              ...
                              {activeResult.breakEvenYears !== null && (
                                <>
                                  <span className="h-6 w-px bg-app" />
                                  <div>
                                    <p className="text-muted uppercase tracking-[0.15em] text-[0.55rem] font-semibold">Payback</p>
                                    <p className="font-mono tabular-nums text-secondary mt-0.5">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {[
                            ...
                            <div
                              key={m.label}
                              className="bg-app-elevated border border-app rounded-xl p-3 text-center"
                            >
                              <p className="text-[0.55rem] uppercase tracking-[0.18em] text-muted font-semibold">
                                {m.label}
                              </p>
                              <p className="font-mono text-2xl font-bold tabular-nums text-tool-accent mt-1.5">
                                {m.val.toFixed(2)}
                                <span className="text-base text-muted">%</span>
                              </p>
                              <p className="text-[0.55rem] text-muted mt-1.5 leading-tight">
                                {m.hint}
                              </p>
                            </div>
                          ))}
                        ...
                        <div className="bg-app-elevated border border-app rounded-xl overflow-hidden">
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="p-5 space-y-1 text-[0.78rem]">
                                <div className="flex justify-between">
                                  <span className="text-secondary">Gross rental</span>
                                  <span className="font-mono tabular-nums text-app">{fmt(grossAnnualRent)}</span>
                                </div>
                                <div className="flex justify-between">
                                ...
                                <div className="flex justify-between border-t border-app pt-2 font-semibold">
                                  ...
                                  <span className="font-mono tabular-nums text-tool-accent">{fmt(effectiveRent)}</span>
                                </div>
                                <div className="pt-3 space-y-1">
                                  <div className="flex justify-between">
                                    <span className="text-secondary">Service charge</span>
                                    <span className="font-mono tabular-nums text-rose-500">({fmt(service)})</span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-secondary">Maintenance</span>
                                    <span className="font-mono tabular-nums text-rose-500">({fmt(maintenance)})</span>
                                  </div>
                                  {annualMortgage > 0 && (
                                    <div className="flex justify-between">
                                      ...
                                      <span className="font-mono tabular-nums text-rose-500">({fmt(annualMortgage)})</span>
                                    </div>
                                  )}
                                  <div className="flex justify-between border-t border-app pt-2 font-semibold">
                                    <span className="text-app">Total expenses</span>
                                    <span className="font-mono tabular-nums text-rose-500">({fmt(totalExpenses)})</span>
                                  </div>
                                </div>
                                <div className="bg-tool-accent-soft px-3 py-2.5 rounded-lg mt-3">
                                  <div className="flex justify-between items-baseline">
                                    <span className="text-[0.7rem] uppercase tracking-[0.15em] font-semibold text-tool-accent">
                                      Net annual income
                                    </span>
                                    <span
                                  ...
                                  <div className="flex justify-between text-[0.7rem] mt-1.5 pt-1.5 border-t border-tool-accent/20">
                        ...
                        <div className="bg-app-elevated border border-app rounded-xl overflow-hidden">
                          ...
                          <div className="grid grid-cols-4 border-b border-app text-[0.55rem] uppercase tracking-[0.14em] text-muted font-semibold">
                          ...
                          {activeResult.timeline.map((yd, yi) => (
                            <div
                              key={yd.year}
                              className={`grid grid-cols-4 text-[0.72rem] font-mono tabular-nums transition-colors hover:ring-1 hover:ring-tool-accent hover:ring-inset ${
                        ...
                        {/* Cross-tool CTAs - state buttons w/ window.location, no Link */}
                        <motion.div
                        ...
                        >
                          <button
                            type="button"
                            onClick={() => {
                              window.location.href = "/tools/deal-scoring";
                            }}
                            className="border border-app bg-app-elevated px-4 py-3 text-[0.65rem] uppercase tracking-[0.14em] text-tool-accent font-semibold text-center transition-all duration-200 hover:bg-tool-accent-soft rounded-lg"
                          >
                            Score This Deal →
                          </button>
                          <button
                            type="button"
                            onClick={() => {
                              window.location.href = "/tools/yield-heatmap";
                            }}
                            className="border border-app bg-app-elevated px-4 py-3 text-[0.65rem] uppercase tracking-[0.14em] text-tool-accent font-semibold text-center transition-all duration-200 hover:bg-tool-accent-soft rounded-lg"
                          >
                            Yield Heatmap →
              ...
              {!isFramed && (
                <div className="mt-10">
                  <ToolRecommendations slug="roi-calculator" />
                </div>
              )}
            </div>
          </section>
