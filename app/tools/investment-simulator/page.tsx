...
export default function InvestmentSimulatorPage() {
  return (
    <Suspense fallback={null}>
      <InvestmentSimulatorInner />
    </Suspense>
  );
}

function InvestmentSimulatorInner() {
    ...
    <AuthGate>
      ...
      >
        {!isFramed && (
          <PageHeader
            label="Tool"
            title="Property Investment Simulator"
            description="What if you bought property in Dubai? See actual returns based on historical data, with forward projections to 2030."
            color="purple"
          />
        )}
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-5 pb-16">
          ...
          <motion.section
            ...
            className="relative overflow-hidden rounded-2xl border border-tool-accent/30 bg-tool-accent-soft p-6 sm:p-8 mb-6"
          >
          ...
          {!isFramed && <ToolRecommendations slug="investment-simulator" />}
        </div>
      </div>
    </AuthGate>
  );
