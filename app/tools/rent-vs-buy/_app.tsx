...
/* ═══════════════════════════════════════════════════════════════════════════
   Rent vs Buy — Native Desktop Workspace App
   ───────────────────────────────────────────────────────────────────────────
   Inside-the-Window variant of `/tools/rent-vs-buy`. The standalone page
   (page.tsx) stays for direct visits / SEO / sharing. This file is mounted
   by the desktop's Window component when the user opens the tool from the
   workspace.

   Differences vs page.tsx:
     - No <AuthGate> (workspace handles auth at the shell level).
     - No frame=1 / Suspense+useSearchParams (Window provides its own chrome
       and passes intent via NativeAppProps).
     - No PageHeader / back-links to /tools.
     - No <ToolRecommendations> at the foot.
     - Cross-tool jumps go via props.openApp(slug, params).
     - Layout collapses based on the live window width, not the viewport.
     - Canvas tone derives from `resolved` so dark/light themes draw correctly.

   All NPV math, mortgage payment, alt-investment compounding, RERA/DLD/Golden
   Visa logic and feature surface are preserved verbatim from page.tsx.
═══════════════════════════════════════════════════════════════════════════ */
...
export default function RentVsBuyApp(props: NativeAppProps) {
  const { width, resolved, initialParams, initialParamsKey } = props;
  ...
  /* Hydrate from initialParams for cross-tool jumps. */
  useEffect(() => {
    if (!initialParams) return;
    const setStr = (key: string, setter: (v: string) => void) => {
      const v = initialParams[key];
      if (typeof v === "number") setter(String(v));
      else if (typeof v === "string") setter(v);
    };
    setStr("monthlyRent", setMonthlyRent);
    setStr("annualRentIncrease", setAnnualRentIncrease);
    setStr("renterInsurance", setRenterInsurance);
    setStr("purchasePrice", setPurchasePrice);
    setStr("downPaymentPct", setDownPaymentPct);
    setStr("mortgageRate", setMortgageRate);
    setStr("mortgageTerm", setMortgageTerm);
    setStr("appreciation", setAppreciation);
    setStr("serviceCharge", setServiceCharge);
    setStr("annualMaintenance", setAnnualMaintenance);
    setStr("altReturn", setAltReturn);
    if (typeof initialParams.horizon === "number") setHorizon(initialParams.horizon);
    const r = initialParams.residency;
    if (r === "uae-national" || r === "resident" || r === "non-resident") {
      setResidency(r);
    }
  ...
  const inputColsClass = isWide ? "lg:grid-cols-2" : "";
  ...
  return (
    <div
      ...
      className="h-full w-full overflow-auto bg-app text-primary"
    >
      {/* ═════════ STYLE — slider + input ═════════ */}
      <style jsx global>{`
      ...
      <div className={`mx-auto max-w-6xl ${containerPad}`}>
        {/* ═════════ COMPACT IN-WINDOW HEADER ═════════ */}
        <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="relative flex h-10 w-10 items-center justify-center rounded-2xl bg-tool-accent-soft ring-1 ring-tool-accent">
              <span
                className="rvb-pulse absolute h-1.5 w-1.5 rounded-full bg-tool-accent"
                aria-hidden
              />
              <svg
                viewBox="0 0 24 24"
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] sm:text-xs text-secondary">
                NPV crossover · alt-return opportunity cost · RERA, DLD &amp; Golden Visa.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {breakeven !== null && (
              <span className="inline-flex items-center gap-1.5 rounded-md border border-tool-accent bg-tool-accent-soft px-2.5 py-1 font-mono text-[11px] text-tool-accent">
                <span className="h-1.5 w-1.5 rounded-full bg-tool-accent" />
                Crossover Y{breakeven}
              </span>
            )}
            <span className="inline-flex items-center gap-1.5 rounded-md border border-app bg-tool-surface px-2.5 py-1 font-mono text-[11px] text-muted">
              <span className="h-1.5 w-1.5 rounded-full bg-tool-accent" />
              {horizon}-yr horizon
            </span>
          </div>
        ...
        <AnimatePresence>
          {hasResult && lastRow && (
            ...
            >
              ...
              <div className="flex flex-wrap items-end justify-between gap-6">
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap items-baseline gap-3">
                    <h2
                      className={`font-semibold tracking-tight text-tool-accent ${
              ...
              <div className={`mt-6 grid gap-3 ${wealthCardsCols}`}>
                <div
                ...
                >
                  ...
                  <p
                    className={`mt-2 font-mono font-semibold tabular-nums ${
                      isUltra ? "text-2xl" : "text-3xl sm:text-4xl"
                    } ${
                  ...
                  <p className="mt-1 font-mono text-[0.65rem] tabular-nums text-muted">
                    Cum cost {fmtCompact(lastRow.rentCostCumulative)} · pool{" "}
                    {fmtCompact(lastRow.investmentValue)}
                  </p>
                </div>
                <div
                  className={`rounded-2xl border p-5 transition-colors ${
                ...
                >
                  ...
                  <p className="mt-1 font-mono text-[0.65rem] tabular-nums text-muted">
                    Equity {fmtCompact(lastRow.equity)} · cum cost{" "}
                    {fmtCompact(lastRow.buyCostCumulative)}
                  </p>
                </div>
              </div>
            </motion.section>
        ...
        >
          <div className={`grid gap-6 ${horizonGridCols}`}>
            <div>
        ...
        {/* ═════════ INPUT GRID — RENT + BUY ═════════ */}
        <div className={`grid gap-6 ${inputColsClass}`}>
          {/* Rent */}
          <motion.section
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className={`grid ${buyDetailCols} gap-3 border-t border-app pt-4`}>
                <div>
          ...
          {hasResult && lastRow && (
            ...
            >
              ...
              >
                ...
                <CrossoverChart data={timeline} breakeven={breakeven} tone={tone} />
              </motion.section>
                          ...
                          >
                            <td
                              className={`px-3 py-1.5 ${
                                isCrossover ? "text-tool-accent font-semibold" : "text-muted"
                              }`}
                            >
                              {row.year}
                              {isCrossover && (
                            ...
                            <td className="px-3 py-1.5 text-right">
                              {fmtCompact(row.rentCostCumulative)}
                            </td>
                            <td className="px-3 py-1.5 text-right">
                              {fmtCompact(row.buyCostCumulative)}
                            </td>
                            <td
                              className={`px-3 py-1.5 text-right ${
                                row.rentNetWealth >= row.buyNetWealth
                            ...
                            <td
                              className={`px-3 py-1.5 text-right ${
                                row.buyNetWealth >= row.rentNetWealth
                            ...
                            <td className="px-3 py-1.5 text-right text-muted">
                              {fmtCompact(row.propertyValue)}
                            </td>
                            <td className="px-3 py-1.5 text-right text-muted">
                              {fmtCompact(row.equity)}
                            </td>
                          </tr>
                        );
                      })}
              ...
              >
                <div className="mb-4">
                  <h3 className="text-sm font-semibold tracking-tight">UAE-Specific Insights</h3>
                  <p className="font-mono text-[0.65rem] uppercase tracking-[0.2em] text-muted">
