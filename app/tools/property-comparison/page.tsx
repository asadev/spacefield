...
function computeScore(p: PropertyInput, allProperties: PropertyInput[]): number {
  ...
  // Normalize each metric 0-100 among properties
...
// ─────────────────────────────────────────────────────────────────────────────
// Image upload — native-style placeholder using tool-accent stub
// ─────────────────────────────────────────────────────────────────────────────
function PhotoSlot({
  image,
  onUpload,
...
// ─────────────────────────────────────────────────────────────────────────────
// Score dial — circular SVG ring stroked in tool-accent
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Property card — native, side-by-side
// ─────────────────────────────────────────────────────────────────────────────
function PropertyCard({
  ...
  return (
    ...
    >
      ...
      {/* Key-metric grid (price · sqft · beds · yield) */}
...
// ─────────────────────────────────────────────────────────────────────────────
// Edit drawer — local state form sheet, slides in from right
// ─────────────────────────────────────────────────────────────────────────────
function EditDrawer({
  property,
  onClose,
...
// Main page
// ─────────────────────────────────────────────────────────────────────────────
function PropertyComparisonInner({ frame }: { frame: boolean }) {
  ...
  // Card grid: side-by-side, scales from 2 → 4 columns
  const cardGridCols =
    properties.length === 2
  ...
  return (
    <div data-tool-theme="research" data-tool="property-comparison" className="tool-shell bg-app">
      {!frame && (
        <PageHeader
          label="Tool"
          title="Property Comparison"
          description="Compare up to 4 properties side-by-side with images, scoring, and detailed metrics. Auto-highlights the best option per metric."
          color="amber"
        />
      )}
      ...
      >
        ...
        >
          {/* In-tool app header */}
          <div className={`tool-hero relative overflow-hidden border-b border-app ${frame ? "" : "rounded-t-xl"}`}>
            <div
              aria-hidden
          ...
          <AnimatePresence mode="wait">
            {showComparison && (
              ...
              >
                {/* Tabs */}
                <div className="flex items-center gap-1 border-b border-app bg-tool-accent-soft/40 px-4 py-2 sm:px-6">
                ...
                <p className="border-t border-app px-4 py-3 text-[10px] leading-relaxed text-muted sm:px-6">
        ...
        {!frame && <ToolRecommendations slug="property-comparison" />}
      </section>
...
// ─────────────────────────────────────────────────────────────────────────────
// Comparison matrix — rows = attributes, cols = properties
// ─────────────────────────────────────────────────────────────────────────────
function ComparisonMatrix({
  ...
  const colTemplate = `minmax(160px, 200px) repeat(${properties.length}, minmax(0, 1fr))`;
...
// ─────────────────────────────────────────────────────────────────────────────
// Scores panel — score dial per property + ranked list
// ─────────────────────────────────────────────────────────────────────────────
function ScoresPanel({
  ...
  return (
    <div className="space-y-5 px-4 py-5 sm:px-6">
      {/* Dial row */}
      <div className={`grid gap-3 ${properties.length === 2 ? "grid-cols-2" : properties.length === 3 ? "grid-cols-3" : "grid-cols-2 lg:grid-cols-4"}`}>
        {properties.map((p, i) => (
          <div
            key={p.id}
...
function PropertyComparisonWithFrame() {
  const searchParams = useSearchParams();
  const frame = searchParams?.get("frame") === "1";
  return <PropertyComparisonInner frame={frame} />;
}

export default function PropertyComparison() {
  return (
    <AuthGate>
      <Suspense fallback={<PropertyComparisonInner frame={false} />}>
        <PropertyComparisonWithFrame />
      </Suspense>
    </AuthGate>
  );
}
