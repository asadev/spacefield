...
/* ═══════════════════════════════════════════════════════════════════════════
   Portfolio Tracker — Native Desktop Workspace App
   ───────────────────────────────────────────────────────────────────────────
   Inside-the-Window variant of `/tools/portfolio-tracker`. The standalone
   ...
   Differences vs page.tsx:
     ...
     - No frame=1 / Suspense + useSearchParams (Window provides its own
       chrome and passes intent via NativeAppProps).
     ...
     - localStorage persistence preserved verbatim.

   Every metric, canvas, sort, allocation toggle, and form field is
   preserved exactly.
═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════
   NATIVE APP
   ═══════════════════════════════════════════════════ */
export default function PortfolioTrackerApp(props: NativeAppProps) {
  const { width } = props;
  ...
  useEffect(() => {
    ...
    const cx = size / 2;
    const cy = size / 2;
    const outerR = size === 220 ? 95 : 120;
    const innerR = size === 220 ? 60 : 78;
  ...
  }, [allocationData, properties.length, tone, isUltra]);
  ...
  useEffect(() => {
    ...
    const dpr = window.devicePixelRatio || 1;
    // Width-aware chart sizing — chart fills the column.
    const w = Math.max(280, Math.min(720, width - (isUltra ? 56 : 96)));
    const h = isUltra ? 200 : 240;
    canvas.width = w * dpr;
    canvas.height = h * dpr;
  ...
  }, [properties, metrics, tone, width, isUltra]);
  ...
  /* Width-driven grids — denser on wide windows. */
  const kpiGridCols = isUltra
    ? "grid-cols-2"
    : isWide
    ? "grid-cols-4"
    : "grid-cols-2";

  const cardsGridCols = isUltra
    ? "grid-cols-1"
    : isNarrow
    ? "grid-cols-2"
    : isWide
  ...
  return (
    ...
    >
      ...
      <div aria-hidden className="absolute inset-x-0 top-0 h-[280px] tool-hero opacity-50 pointer-events-none" />
      <div
        aria-hidden
        className="absolute inset-x-0 top-0 h-[280px] pointer-events-none opacity-[0.035]"
        style={{
          backgroundImage:
      ...
      <section className={`relative z-10 mx-auto ${isUltra ? "px-3 pt-4 pb-10" : "px-4 sm:px-6 pt-5 pb-12"} max-w-6xl`}>
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-baseline gap-3 flex-wrap">
                <div>
                  ...
                  <h1 className={`${isUltra ? "text-3xl" : "text-4xl sm:text-5xl"} font-semibold tracking-tight font-mono tabular-nums text-app`}>
        ...
        <motion.div
          ...
          className={`grid ${kpiGridCols} gap-3`}
        >
          {[
            {
            ...
            >
              ...
              <p className={`mt-2 ${isUltra ? "text-xl" : "text-2xl"} font-semibold text-app font-mono tabular-nums`}>
                {k.value}
              </p>
        ...
        {properties.length > 0 && (
          <motion.div
            ...
            className={`mt-4 grid gap-3 ${isNarrow ? "grid-cols-1" : "grid-cols-5"}`}
          >
            {/* Allocation donut card */}
            <div className={`relative overflow-hidden rounded-xl border border-app bg-app-elevated p-5 ${isNarrow ? "" : "col-span-2"}`}>
            ...
            <div className={`relative overflow-hidden rounded-xl border border-app bg-app-elevated p-5 ${isNarrow ? "" : "col-span-3"}`}>
              <div className="flex items-center justify-between mb-4">
                <div>
        ...
        {properties.length > 0 && (
          ...
          >
            ...
            <div className={`grid ${cardsGridCols} gap-3`}>
            ...
            {/* Holdings table fallback (sortable) — only on wider windows */}
            {isWide && (
              <div className="mt-4 overflow-hidden rounded-xl border border-app bg-app-elevated">
      ...
      {/* ── Form drawer (slides in from right, scoped to the window) ── */}
      <AnimatePresence>
        {showForm && (
          <>
            <motion.div
            ...
            <motion.aside
              ...
              className="absolute inset-y-0 right-0 z-50 w-full max-w-xl overflow-y-auto bg-app-elevated border-l border-app shadow-2xl"
            >
