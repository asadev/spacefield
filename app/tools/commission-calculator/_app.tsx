...
/* ═══════════════════════════════════════════════════════════════════════════
   Commission Calculator — Native Desktop Workspace App
   ───────────────────────────────────────────────────────────────────────────
   ...
   chrome. Layout collapses to single column when width < 900px. All
   commission math, hooks and state are preserved verbatim from page.tsx.
═══════════════════════════════════════════════════════════════════════════ */
...
export default function CommissionCalculatorApp(props: NativeAppProps) {
  ...
  /* Hydrate from initialParams when openApp() passes a deal context. */
  useEffect(() => {
    if (!initialParams) return;
    if (typeof initialParams.transactionValue === "number") {
      setForm((prev) => ({
        ...prev,
        transactionValue: String(Math.round(initialParams.transactionValue as number)),
      }));
    } else if (typeof initialParams.transactionValue === "string") {
      setForm((prev) => ({
        ...prev,
        transactionValue: (initialParams.transactionValue as string).replace(
          /[^0-9.]/g,
          ""
        ),
      }));
    ...
    if (
      initialParams.dealType === "sale-secondary" ||
      initialParams.dealType === "sale-offplan" ||
      initialParams.dealType === "rental-annual" ||
      initialParams.dealType === "rental-monthly"
    ) {
      const dt = initialParams.dealType as DealType;
      const found = DEAL_TYPES.find((d) => d.value === dt);
      const rate = found
        ? dt === "rental-monthly"
          ? "100"
          : String(found.defaultRate)
        : "2";
      setForm((prev) => ({ ...prev, dealType: dt, commissionRate: rate }));
    }
    if (typeof initialParams.commissionRate === "number") {
      setForm((prev) => ({
        ...prev,
        commissionRate: String(initialParams.commissionRate),
      }));
    }
  ...
  return (
    ...
    >
      <style jsx global>{`
        [data-tool="commission-calculator"] .cc-mono {
          font-family: ui-monospace, SFMono-Regular, "JetBrains Mono", Menlo,
            Consolas, monospace;
          font-variant-numeric: tabular-nums;
          letter-spacing: -0.02em;
        }
        [data-tool="commission-calculator"] .cc-input {
        ...
        [data-tool="commission-calculator"] .cc-input::placeholder {
          color: var(--text-faint);
        }
        [data-tool="commission-calculator"] .cc-input:focus {
          outline: none;
          border-color: var(--tool-accent);
          box-shadow: 0 0 0 3px var(--tool-accent-ring);
        }
        [data-tool="commission-calculator"] .cc-range {
        ...
        [data-tool="commission-calculator"] .cc-range::-webkit-slider-thumb {
          -webkit-appearance: none;
          appearance: none;
          width: 20px;
          height: 20px;
        ...
        [data-tool="commission-calculator"] .cc-range::-moz-range-thumb {
          width: 20px;
          height: 20px;
          border-radius: 999px;
          background: var(--bg-elevated);
      ...
      <div className="px-4 pb-10 pt-5 sm:px-6">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <motion.p
                ...
                className="cc-mono mt-2 text-3xl font-bold sm:text-4xl lg:text-5xl"
              >
            ...
            <div
              ...
              className="inline-flex rounded-full border border-app bg-app p-1"
            >
        ...
        {/* ═════════ TWO-COLUMN WORKSPACE — width-driven ═════════ */}
        <div
          className={isWide ? "mt-6 grid gap-6" : "mt-6 flex flex-col gap-5"}
          style={
            isWide
