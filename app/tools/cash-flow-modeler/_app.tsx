...
/* ═══════════════════════════════════════════════════════════════════════════
   Cash Flow Modeler — Native Desktop Workspace App
   ───────────────────────────────────────────────────────────────────────────
   Inside-the-Window variant of `/tools/cash-flow-modeler`. The standalone
   ...
   Differences vs page.tsx:
     ...
     - Cross-tool jumps (when added later) go via props.openApp(slug, params).
     - Layout collapses based on the live window width, not the viewport.

   All NOI/IRR math, hooks, and feature surface are preserved verbatim.
═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════
   NATIVE APP
   ═══════════════════════════════════════════════════ */
export default function CashFlowModelerApp(props: NativeAppProps) {
  ...
  useEffect(() => {
    ...
    setInputs((prev) => {
      ...
      for (const k of numericKeys) {
        ...
        if (typeof v === "number") (next[k] as string) = String(v);
        else if (typeof v === "string") (next[k] as string) = v;
      }
      if (typeof initialParams.projectionYears === "number") {
        next.projectionYears = initialParams.projectionYears;
      }
      if (typeof initialParams.mortgageEnabled === "boolean") {
        next.mortgageEnabled = initialParams.mortgageEnabled;
      }
      return next;
    });
  ...
  }, [yearData, tone, activeYear, width]);
  ...
  /* Width-driven grid templates — keep dense layouts when there's room. */
  const assumptionsGridCols = isUltra
  ...
  const sensitivityGridCols = isUltra
    ...
    >
      <div className={`mx-auto max-w-6xl ${isMobile ? "px-4 pt-4 pb-12" : isUltra ? "px-3 pt-4 pb-10" : "px-4 sm:px-6 pt-5 pb-12"} space-y-6`}>
        ...
        <motion.section
          ...
          className={`relative overflow-hidden rounded-2xl border border-tool-accent bg-tool-accent-soft ${
        ...
        >
            ...
            <div className="min-w-0">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <span
                    className={`${
                      isUltra ? "text-3xl" : "text-4xl sm:text-6xl"
          ...
          {activeYearData && (
            <div
              className="mt-6 grid gap-3 border-t border-tool-accent pt-4"
              style={{
        ...
        <section
          ...
          style={{ gridTemplateColumns: kpiGridCols }}
        >
          {[
        ...
        {activeYearData && (
          <section className="rounded-xl border border-app bg-app-elevated">
            ...
            <div
              className={`grid divide-app ${
                isWide ? "grid-cols-2 divide-x" : "grid-cols-1 divide-y"
              }`}
            >
              {/* Income column */}
              <div className="p-5">
        ...
        <section>
          ...
          <div
            className="grid gap-3"
            style={{ gridTemplateColumns: sensitivityGridCols }}
          >
            {[
