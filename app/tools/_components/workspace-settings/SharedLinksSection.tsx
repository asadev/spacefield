...
/* ─────────────────────────────────────────────────────────────────────────
 * SharedLinksSection — manage every toshare.net link minted from this
...
import type { ToShareLinkRow, ToShareType } from "@/lib/toshare/types";
import { buildToShareUrl, TOSHARE_TYPE_PREFIX } from "@/lib/toshare/types";
...
const TYPE_LABEL: Record<ToShareType, string> = {
  form: "Form",
  page: "Page",
  quote: "Quote",
  booking: "Booking",
...
const TYPE_COLOR: Record<ToShareType, string> = {
...
export default function SharedLinksSection({ workspaceId, workspaceLabel }: Props) {
  const [links, setLinks] = useState<ToShareLinkRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState<"all" | ToShareType>("all");
  ...
  async function toggleDeliveries(linkId: string) {
    ...
    if (!deliveries[linkId]) {
      ...
      try {
        const res = await fetch(`/api/toshare/links/${linkId}/deliveries`, {
          cache: "no-store",
        });
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    fetch(`/api/toshare/links?workspaceId=${encodeURIComponent(workspaceId)}`, {
      cache: "no-store",
    })
      .then((r) => r.json())
      .then((j) => {
  ...
  async function copyUrl(link: ToShareLinkRow) {
  ...
  async function toggleStatus(link: ToShareLinkRow) {
    setBusy(link.id);
    const newStatus = link.status === "active" ? "paused" : "active";
    const action = newStatus === "active" ? "resume" : "pause";
    try {
      await fetch(`/api/toshare/links/${link.id}/${action}`, { method: "POST" });
      setLinks((prev) =>
        prev.map((l) =>
          l.id === link.id ? { ...l, status: newStatus as ToShareLinkRow["status"] } : l
        )
      );
    } finally {
      setBusy(null);
  ...
  async function destroy(link: ToShareLinkRow) {
    if (!confirm(`Delete link toshare.net/${TOSHARE_TYPE_PREFIX[link.type]}/${link.slug}? This is permanent.`)) {
      return;
    }
    setBusy(link.id);
    try {
      await fetch(`/api/toshare/links/${link.id}`, { method: "DELETE" });
  ...
  function getTitle(link: ToShareLinkRow): string {
      ...
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-faint">
            ...
            form, page, quote, booking, redirect, or file hosted at toshare.net.
          </p>
        </div>
        <button
          type="button"
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-xs text-faint">
                  ...
                  and comparing the <code>X-toShare-Signature</code> header.
                </div>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
...
function verify(req, secret) {
  const sig = req.headers["x-toshare-signature"]?.replace(/^sha256=/, "");
  ...
  return sig && crypto.timingSafeEqual(
    ...
    Buffer.from(expected, "hex"),
      ...
      <div className="flex flex-wrap items-center gap-2">
        <select
          value={filter}
          onChange={(e) => setFilter(e.target.value as "all" | ToShareType)}
