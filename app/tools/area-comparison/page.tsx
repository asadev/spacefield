...
const METRICS: MetricRow[] = [
  ...
  },
  {
    ...
    group: "Price",
    getValue: (c) => `${fmt(c.pricing.pricePerSqft.min)} – ${fmt(c.pricing.pricePerSqft.max)}`,
...
export default function AreaComparison() {
  ...
  const canCompare =
    selections.filter((s) => s !== null).length >= 2;
  ...
  const gridCols =
    compared.length === 3
      ? "grid-cols-[160px_1fr_1fr_1fr] sm:grid-cols-[200px_1fr_1fr_1fr]"
      : "grid-cols-[160px_1fr_1fr] sm:grid-cols-[200px_1fr_1fr]";
  ...
  return (
    <AuthGate>
      ...
      >
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 pb-10">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                ...
                <h1 className="font-tool-heading text-3xl sm:text-4xl font-semibold text-app tracking-tight">
                  Area Comparison
                </h1>
            ...
            {/* In-page tab strip — replaces all <Link> sub-navigation */}
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-24 pt-8">
          {/* Area picker — side-by-side cards */}
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.4, ease }}
            className="rounded-2xl border border-app bg-app-elevated p-5 sm:p-6 mb-6"
          >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-base sm:text-lg text-app font-medium font-tool-heading">
                ...
                {compared.length >= 2 && (
                  ...
                  >
                    <div className="rounded-2xl border border-app bg-app-elevated overflow-hidden mb-6">
                      ...
                      <div
                        className={`grid border-b border-app ${gridCols}`}
                      >
                        ...
                        {compared.map((c, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                className="font-tool-heading font-semibold text-sm sm:text-base text-app"
                                style={{ color: SWATCHES[i] }}
                              >
                                {c.name}
                              </div>
                      ...
                      <div className="p-6 border-b border-app">
                        ...
                        <RadarChart items={compared} scoresMap={scoresMap} />
                      </div>
                      ...
                      {METRIC_GROUPS.map((group) => {
                        ...
                        return (
                          <div key={group}>
                            {/* Group header */}
                            <div
                              className={`grid ${gridCols} bg-app border-b border-app`}
                            >
                              <div className="px-4 py-2.5 text-[0.6rem] uppercase tracking-[0.3em] text-tool-accent">
                                {group}
                              </div>
                              <div
                                className={`col-span-${compared.length} border-l border-app`}
                              />
                            </div>
                            {rows.map((metric, mIdx) => {
                              const wl = getWinnerLoser(metric, compared);
                              // striped row alternation by index across the whole table
                              const striped = mIdx % 2 === 1;
                              return (
                                <div
                                  key={metric.label}
                                  className={`grid ${gridCols} border-b border-app transition-colors ${
                                    striped ? "bg-app" : "bg-app-elevated"
                                  }`}
                                >
                                  <div className="p-3 sm:p-4 text-xs sm:text-sm text-secondary flex items-center">
                                    {metric.label}
                                  </div>
                                  {compared.map((c) => {
                                    ...
                                    return (
                                      <div
                                        key={c.id}
                                        className={`p-3 sm:p-4 border-l border-app text-center text-xs sm:text-sm relative ${
                                          isWinner
                            ...
                            <>
                              <div className="flex items-start gap-3">
                                ...
                                <div className="text-sm text-secondary">
                                  ...
                                  {investBest.investment.capitalAppreciation3y.min}%
                                  –
                                  {investBest.investment.capitalAppreciation3y.max}
                                  %.
                                </div>
                              </div>
                              <div className="flex items-start gap-3">
          ...
          <ToolRecommendations slug="area-comparison" />
        </section>
      </div>
    </AuthGate>
  );
