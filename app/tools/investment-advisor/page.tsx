...
export default function InvestmentAdvisorPage() {
  return (
    <Suspense fallback={null}>
      <InvestmentAdvisorInner />
    </Suspense>
  );
}

function InvestmentAdvisorInner() {
  ...
  const renderResults = () => {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease }} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {results.slice(0, 2).map((area) => (
              <div key={area.area} className="space-y-2">
                ...
                <Link href="/tools/yield-heatmap" className="block text-sm text-tool-accent hover:underline transition-colors">
                  Check yield in {area.area} →
                </Link>
                <Link href="/tools/deal-scoring" className="block text-sm text-tool-accent hover:underline transition-colors">
                  Score a deal in {area.area} →
                </Link>
                <Link href="/tools/neighborhood-report" className="block text-sm text-tool-accent hover:underline transition-colors">
            ...
            <div className="space-y-2">
              ...
              <Link href="/tools/mortgage-calculator" className="block text-sm text-tool-accent hover:underline transition-colors">
                Calculate your mortgage →
              </Link>
              <Link href="/tools/area-comparison" className="block text-sm text-tool-accent hover:underline transition-colors">
                Compare areas side by side →
              </Link>
              <Link href="/tools/golden-visa-checker" className="block text-sm text-tool-accent hover:underline transition-colors">
        ...
        <ToolRecommendations slug="investment-advisor" />
      </motion.div>
    );
  };
  ...
  /* ───── Wizard Body ───── */
  const wizardBody = (
    <section className={`max-w-3xl mx-auto px-4 ${frameMode ? "pt-4" : "pt-2"} pb-24`}>
  ...
  return (
    <AuthGate>
      <div data-tool-theme="investment" data-tool="investment-advisor" className="min-h-screen bg-app">
        {!frameMode && (
          <PageHeader
            label="Tools"
            title="AI Investment Advisor"
            description="Answer a few questions about your goals, budget, and risk tolerance — get a personalized Dubai real estate investment strategy with specific area and property recommendations."
            color="purple"
          />
        )}
        {wizardBody}
