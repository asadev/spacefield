 ...
 *
 * TOSHARE-01: this used to call redirect(payload.url) with zero validation,
 * turning trusted toshare.net/r/<slug> into an open-redirect / phishing
 * relay. We now (1) re-validate the stored URL here as defence-in-depth
 * (mint already rejects bad URLs — see lib/toshare/server.ts) and reject
...
import { isSafeRedirectUrl, type RedirectPayload } from "@/lib/toshare/types";
...
export default async function RedirectViewer({ params, searchParams }: Props) {
  ...
  // TOSHARE-01: never forward to an unvalidated / non-http(s) destination.
    ...
    <>
      ...
      <article className="space-y-6">
        ...
        <p className="text-sm text-slate-600">
          You are leaving toshare.net and being forwarded to{" "}
