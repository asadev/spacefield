...
/* Contact Manager — native window app.
 *
 * Same data + feature set as the route page but stripped of marketing chrome:
 * no ToolShell / AuthGate / WorkspaceSwitcher / ToolRecommendations / page
 * frame / back-links. Pure tool surface, sized to the window's width/height.
 *
 * Layout adapts off `width` (desktop window inner width):
 *   ≥ 1000 → list rail + main pane side by side (340 + flex)
 *   <  1000 → stacked (rail on top, then pane)
 *
 * All colors come from foundation tokens. data-tool-theme="crm" scopes the
 * accent. Workspace mode (personal vs team) is preserved via useWorkspace.
 */

import { useEffect, useMemo, useRef, useState } from "react";
import type { NativeAppProps } from "../../../tools/_data/tools-list";
import { Field, Stat, inputCls } from "../../_components/ToolCard";
import {
...
function getActivityCountsFromTimeline(): Map<
  string,
...
function relationshipStrength(
  ...
  const freq = Math.min(1, activityCount / 15);

  let recency = 0;
  if (lastAtIso) {
    const days = Math.max(
      0,
      (Date.now() - new Date(lastAtIso).getTime()) / 86400000
    );
    if (days < 7) recency = 1;
    ...
    contacts: [
      {
        ...
        status: "qualified",
        notes:
          "Met at SaaStr. Expanding EMEA team, needs pipeline tooling.",
...
function toVCard(contacts: Contact[]): string {
  ...
  return contacts
    .map((c) => {
      ...
      const first =
        parts.length > 1 ? parts.slice(0, -1).join(" ") : c.name;
      ...
      if (c.tags.length)
        lines.push(`CATEGORIES:${c.tags.map(esc).join(",")}`);
...
/* ═════════════════════════════ MAIN APP ═════════════════════════════ */
export default function ContactManagerApp({ width }: NativeAppProps) {
  ...
  const statCols = width >= 720 ? "grid-cols-5" : "grid-cols-2";
  const headerActionsWrap = width < 720;
  const detailsCols = width >= 720 ? "md:grid-cols-2" : "grid-cols-1";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (current.kind === "team") {
        ...
        if (cancelled) return;
        setState(
          data && Array.isArray(data.contacts) ? data : defaultState()
        );
      } else {
        try {
  ...
  const mergeDuplicates = () => {
    ...
    alert(
      `Merged ${mergedCount} exact duplicate${mergedCount === 1 ? "" : "s"}.`
    );
  };
  ...
  const fuzzyCandidates = useMemo(() => {
    ...
    for (let i = 0; i < state.contacts.length; i++) {
      for (let j = i + 1; j < state.contacts.length; j++) {
        ...
        const companySim =
          a.company && b.company ? similarity(a.company, b.company) : 0;
  ...
  const importVcf = (file: File) => {
    ...
    reader.onload = () => {
      ...
      setState((s) => ({ contacts: [...add, ...s.contacts] }));
      alert(
        `Imported ${add.length} contact${add.length === 1 ? "" : "s"} from vCard.`
      );
    };
    reader.readAsText(file);
  };
  ...
  return (
    <div
      data-tool-theme="crm"
      data-tool="contact-manager"
      className="h-full w-full overflow-y-auto bg-app p-4 text-app"
    >
      {/* ============================== MASTHEAD ============================== */}
      <section className="relative mb-4 overflow-hidden rounded-xl border border-app bg-app-elevated">
        ...
        <div className="relative p-4">
          <div
            className={`relative flex ${
              headerActionsWrap ? "flex-col" : "flex-wrap"
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-2 text-xl font-semibold tracking-tight text-app md:text-2xl">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <span className="rounded-md border border-app bg-app px-2.5 py-1 font-mono text-[0.65rem] uppercase tracking-[0.16em] text-secondary">
                  {stats.lead}L · {stats.qualified}Q · {stats.customer}C ·{" "}
                  {stats.lost}X
                </span>
                <span className="rounded-md border border-app bg-app px-2.5 py-1 font-mono text-[0.65rem] uppercase tracking-[0.16em] text-secondary">
      ...
      {/* Stats row */}
      <div className={`mb-4 grid gap-3 ${statCols}`}>
      ...
      <div
        className={`grid grid-cols-1 gap-4 ${
          sideBySide ? "lg:grid-cols-[340px_1fr]" : ""
        }`}
      >
        ...
        <aside className="flex flex-col rounded-xl border border-app bg-app-elevated">
          ...
          {view === "grid" ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 w-full">
                      ...
                      <div
                        className={`mt-1 font-mono text-[0.55rem] uppercase tracking-[0.14em] ${rel.cls}`}
                      >
                        {rel.label} · {rel.score}
                      </div>
                    </div>
            ...
            <ul className="max-h-[640px] divide-y divide-app overflow-y-auto">
              {filtered.map((c) => {
                const rel = relFor(c);
                const entry = activityCounts.get(
                  ...
                  <li key={c.id}>
                    <button
                      ...
                      className={`flex w-full items-center gap-3 px-3 py-3 text-left transition-colors ${
                        active ? "bg-tool-accent-soft" : "hover:bg-app"
                      }`}
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`rounded-md border px-1.5 py-0.5 font-mono text-[0.5rem] uppercase tracking-[0.14em] ${statusTone(
                              c.status
                            )}`}
                          >
                            {c.status}
        ...
        <section className="rounded-xl border border-app bg-app-elevated">
          {selectedContact && selectedRel ? (
            <div>
              {/* Header */}
              <div className="flex flex-wrap items-start gap-4 border-b border-app p-5">
                <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-xl bg-tool-accent-soft text-xl font-semibold text-tool-accent">
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="rounded-xl border border-tool-accent bg-tool-accent-soft px-4 py-3 text-right">
                    ...
                    <div
                      className={`font-mono text-[0.6rem] uppercase tracking-[0.15em] ${selectedRel.cls}`}
                    >
                      {selectedRel.label}
                    </div>
                  </div>
              ...
              <div
                className={`grid grid-cols-1 gap-4 border-b border-app p-5 ${detailsCols}`}
              >
                <div>
              ...
              <div className="p-5">
                ...
                <div className={`grid grid-cols-1 gap-3 ${detailsCols}`}>
                  <Field label="Name">
                    <input
            ...
            <div className="flex h-full min-h-[300px] flex-col items-center justify-center bg-app-elevated p-10 text-center">
      ...
      {fuzzyCandidates.length > 0 && (
        <div className="mt-4 rounded-xl border border-app bg-app-elevated p-5">
          <div className="mb-3">
            <div className="text-[0.55rem] uppercase tracking-[0.22em] text-muted">
              {fuzzyCandidates.length} fuzzy match
              {fuzzyCandidates.length === 1 ? "" : "es"} · Levenshtein
              similarity ≥ 82%
            </div>
            ...
            {fuzzyCandidates.map((p, i) => (
              ...
              >
                ...
                <button
                  onClick={() => {
                    if (
                      confirm(`Merge "${p.b.name}" into "${p.a.name}"?`)
                    ) {
