...
/* ═══════════════════════════════════════════════════════════════════════════
   Status Page Generator — Native Desktop Workspace App
   ───────────────────────────────────────────────────────────────────────────
   Receives NativeAppProps from the workspace Window. No ToolShell, no
   AuthGate, no PageHeader, no back-links, no ToolRecommendations, no
   bespoke macOS chrome (Window already provides it). Width-driven —
   collapses to a single column when narrow. The full editor surface
   (Live / Incidents / 90-day uptime tabs, services CRUD, incidents CRUD,
   subscriptions, archive retention, branded HTML export, in-app preview)
   is preserved verbatim from page.tsx.
═══════════════════════════════════════════════════════════════════════════ */
...
export default function StatusPageGeneratorApp(props: NativeAppProps) {
  const { width } = props;
  // Width-driven layout — collapse to single column on narrow windows.
  ...
  return (
    <div
      ...
      className="h-full w-full overflow-y-auto bg-app p-4 md:p-5"
    >
      {/* Hero + tabs panel */}
      <div className="overflow-hidden rounded-2xl border border-app bg-app-elevated">
        {/* Status banner hero */}
        <div className="relative border-b border-app px-4 py-5 md:px-6 md:py-6">
          <div className="flex flex-wrap items-center justify-between gap-5">
            <div className="flex items-center gap-4">
              ...
              >
                <span
                  className={`inline-block h-3 w-3 rounded-full ${STATE_DOT[overall]}`}
                />
              </div>
              <div>
      ...
      {tab === "live" && (
        <div
          className={`mt-5 grid gap-5 ${
            isNarrow ? "grid-cols-1" : "grid-cols-[1.3fr_1fr]"
          }`}
        >
          {/* Components deck */}
          <section className="rounded-2xl border border-app bg-app-elevated">
            ...
            <ul className="divide-y divide-app">
              {state.services.map((svc) => {
                ...
                return (
                  <li
                    key={svc.id}
                    className={`group grid items-center gap-3 px-5 py-3 ${
                      isUltra
                        ? "grid-cols-[auto_1fr_auto]"
                        : "md:grid-cols-[auto_1.5fr_0.8fr_1fr_auto_auto]"
                    }`}
                  >
                    <span
      ...
      {tab === "components" && (
        <section className="mt-5 rounded-2xl border border-app bg-app-elevated">
          <div className="flex items-center justify-between border-b border-app px-5 py-3">
            <div className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-tool-accent" />
              <h2 className="text-[0.7rem] font-semibold uppercase tracking-[0.22em] text-secondary">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`h-1.5 w-1.5 rounded-full ${
                  activeIncidents.length > 0 ? "bg-rose-500" : "bg-tool-accent"
                }`}
              />
              <h2 className="text-[0.7rem] font-semibold uppercase tracking-[0.22em] text-secondary">
                Incident timeline
                        ...
                        <article className="group rounded-lg border border-app bg-app p-3 transition hover:border-tool-accent/40">
                          <div
                            className={`grid gap-2 ${
                              isNarrow
                                ? "grid-cols-1"
                                : "md:grid-cols-[2fr_auto_auto_auto]"
                            }`}
                          >
                            <input
                              value={inc.title}
