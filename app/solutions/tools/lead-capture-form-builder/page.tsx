...
interface FormField {
  ...
  step?: number; // for multi-step forms, 1..n
}
...
interface FormDef {
  ...
  action: string;
  multiStep?: boolean;
  tracking?: boolean; // include analytics snippet on embed
}
...
export default function LeadCaptureFormBuilderPage() {
  return (
    <div data-tool-theme="marketing" data-tool="lead-capture-form-builder">
      <ToolShell
        category="CRM & Sales Ops"
        title="Lead Capture Form Builder"
        description="Design a lead form with 10 industry-ready templates (consultation, demo, pricing, white paper, webinar, newsletter, RSVP, support, trial, contact). Conditional logic, multi-step, embeddable HTML with event tracking."
      >
        <Inner />
      </ToolShell>
    </div>
...
function Inner() {
  ...
  // Fields visible at the current preview step
  ...
  // Heuristic conversion estimate: shorter required-field count = higher est.
  ...
  // Highlight syntax-style tokens for embed code: tag attrs in tool-accent
    ...
    <>
      ...
      {mode === "build" && (
        <>
          ...
          {/* 3-rail builder canvas: palette · drop zone · properties */}
          <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[220px_1fr_320px]">
