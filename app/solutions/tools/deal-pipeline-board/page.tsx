...
export default function DealPipelineBoardPage() {
  return (
    <ToolShell
      category="CRM & Sales Ops"
      title="Deal Pipeline Board"
      description="Kanban-style sales pipeline. Drag deals between stages, track totals per column, and see weighted value. Team mode shares the board."
    >
      <Inner />
    </ToolShell>
  );
...
function Inner() {
  ...
  const openTotal = totals.Prospecting + totals.Qualified + totals.Proposal + totals.Negotiation;
  ...
  return (
    <div data-tool-theme="crm" data-tool="deal-pipeline-board">
      <WorkspaceSwitcher />
          ...
          <div className="relative flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-baseline gap-3">
                <div className="text-3xl font-semibold tracking-tight text-app md:text-4xl tabular-nums">
              ...
              <div className="mt-2 text-sm text-secondary">
                ...
                {stuckDeals.length > 0 && (
                  <>
                    {" "}
                    · <span className="text-rose-500">{stuckDeals.length} stuck</span>
                  </>
                )}
              </div>
            </div>
            ...
            return (
              <div key={stage} className="flex items-center gap-3 text-xs">
                ...
                <div className="relative h-5 flex-1 overflow-hidden rounded border border-app bg-app">
                  <div
                    className={isWon ? "h-full bg-emerald-500/60" : "h-full bg-tool-accent"}
                    style={{
                      width: `${widthPct}%`,
      ...
      {stuckDeals.length > 0 && (
        <div className="mb-5 rounded-xl border border-rose-500/30 bg-rose-500/10 p-3 text-sm">
          <div className="mb-1 font-mono text-[0.55rem] uppercase tracking-[0.2em] text-rose-500">
            Stuck deals ({stuckDeals.length}) — {STUCK_DAYS_THRESHOLD}+ days in stage
          </div>
      ...
      {view === "forecast" ? (
        <ForecastView deals={state.deals} totals={totals} />
      ) : view === "list" ? (
        <ListView
          deals={state.deals}
          update={update}
...
function ListView({
  ...
  return (
    <ToolCard title="All deals" subtitle="Flat list view">
