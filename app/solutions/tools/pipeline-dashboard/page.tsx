...
export default function PipelineDashboardPage() {
  return (
    <ToolShell
      ...
      description="Team-aware weighted sales pipeline. Owner column, stage filter, monthly forecast. Personal mode is local; team mode syncs across the workspace."
    >
      <PipelineInner />
    </ToolShell>
  );
...
function PipelineInner() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (current.kind === "team") {
        const data = await loadWorkspaceDataClient<PipelineState>(
          current.id,
          NAMESPACE,
          DATA_KEY
        );
    ...
    setState((s) => ({
      ...
      deals: s.deals.map((d) => {
        ...
        if (patch.stage && !("probability" in patch)) {
          const defProb =
            STAGES.find((st) => st.key === patch.stage)?.default_prob ?? merged.probability;
  ...
  // funnel opacity ramp per stage index (0 → 1)
  const funnelStageOpacity = (idx: number, total: number) =>
    0.35 + (idx / Math.max(1, total - 1)) * 0.55;

  return (
    <div data-tool-theme="sales" data-tool="pipeline-dashboard" className="space-y-6">
      <WorkspaceSwitcher />
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-app bg-app px-4 py-2.5">
          ...
          <div className="ml-1 flex flex-1 items-center gap-2 font-mono text-[0.6rem] uppercase tracking-[0.18em] text-muted">
            ...
            <span className="text-secondary">
              {current.kind === "team" ? "team" : "personal"}.weighted
            </span>
          </div>
          ...
          <div className="relative flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex items-baseline gap-3">
                <div className="font-mono text-3xl font-semibold tracking-tight text-app md:text-4xl">
                  ${fmt(totals.weighted)}
                </div>
              ...
              <p className="mt-2 max-w-xl text-sm text-secondary">
                ${fmt(totals.open)} open · ${fmt(totals.won)} closed-won · {totals.winRate.toFixed(0)}% win-rate
              </p>
            </div>
      ...
      {view === "overview" && (
        <>
          ...
          <ToolCard
            title="Stages"
            subtitle="Tap a stage to filter the table below"
          >
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
              {stageRollups.map((s, idx) => {
                ...
                return (
                  <button
                    key={s.key}
                    onClick={() => setStageFilter((cur) => (cur === s.key ? "all" : s.key))}
          ...
          >
            <div className="space-y-2.5">
              {funnel.map((row) => (
                <div key={row.key} className="grid grid-cols-[110px_1fr_auto] items-center gap-3">
                  ...
                  <div className="relative h-7 overflow-hidden rounded-lg border border-app bg-app">
                    ...
                    <div className="absolute inset-0 flex items-center justify-between px-2.5 text-[0.6rem] text-app">
                      ...
                      <span className="font-mono text-muted">${fmt(row.value)}</span>
                    </div>
                  </div>
                  ...
                  <div key={month}>
                    ...
                    <div className="relative h-7 overflow-hidden rounded-lg border border-app bg-app">
                      ...
                      <div className="absolute inset-0 flex items-center justify-between px-2 text-[0.6rem] text-app">
                        ...
                        <span className="font-mono text-muted">U ${fmt(v.u)}</span>
                      </div>
                    </div>
                  </div>
                );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-app bg-app-elevated px-4 py-3">
        ...
        <div className="flex items-center gap-4 font-mono text-[0.65rem] text-secondary">
          <span>
            Unweighted <span className="text-app">${fmt(totals.unweighted)}</span>
          </span>
          <span>
            Weighted <span className="text-tool-accent">${fmt(totals.weighted)}</span>
          </span>
          <span>
            Win-rate <span className="text-app">{totals.winRate.toFixed(0)}%</span>
          </span>
        </div>
      </div>
    </div>
...
function DealsTable({
  ...
  const stageOpacity = (idx: number) => 0.35 + (idx / Math.max(1, STAGES.length - 1)) * 0.55;
          ...
          >
            <header className="flex flex-wrap items-center justify-between gap-2 border-b border-app bg-app px-4 py-2.5">
              ...
              <div className="flex items-center gap-3 font-mono text-[0.6rem] text-muted">
                ...
                <span className="text-tool-accent">W ${fmt(stage.weighted)}</span>
              </div>
            </header>
            ...
            <ul className="divide-y divide-[color:var(--border)]">
              {stage.deals.map((d) => {
                ...
                return (
                  <li
                    key={d.id}
                    className="grid grid-cols-1 gap-3 p-3 transition-colors hover:bg-tool-accent-soft sm:grid-cols-[minmax(0,1.6fr)_repeat(4,minmax(0,1fr))_auto] sm:items-center"
                  >
                    {/* Deal name + owner */}
                    ...
                    {/* Stage selector */}
                    <div>
                      <div className="font-mono text-[0.55rem] font-semibold uppercase tracking-[0.15em] text-faint sm:hidden">
                        Stage
                      </div>
                      <select
                        value={d.stage}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-mono text-[0.6rem] text-muted">%</span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        <span className="font-mono text-[0.65rem] text-faint">$</span>
                        <input
                          type="number"
                          value={d.amount}
                          onChange={(e) =>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateDeal(d.id, { expectedClose: e.target.value })}
                        className={inputCls("text-xs")}
                      />
                    </div>
