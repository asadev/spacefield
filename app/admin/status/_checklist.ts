...
export const CHECKLIST: Item[] = [
  ...
  { id: "n-plus-one", category: "perf", phase: "hardening", title: "N+1 query audit on list pages", status: "partial", priority: "P1", effort: "M", notes: "Audit doc shipped 2026-05-15 (B-2): docs/perf/N-PLUS-1.md lists top suspect spots. Actual fixes follow in a focused round." },
  ...
  { id: "scan-sc-003-error-leak", category: "security", phase: "hardening", title: "[SC-003] Raw DB error messages echoed to API clients", status: "partial", priority: "P1", effort: "M", notes: "PARTIAL 2026-05-15 (V-5): new lib/safe-error.ts::safeErrorMessage helper shipped + patched 5 priority routes (comments x4, tasks x3, projects x3). ~25 other API routes still echo raw err.message — sweep is a follow-up." },
  ...
  { id: "scan-sc-005-pii-eid-plaintext", category: "security", phase: "hardening", title: "[SC-005] employee_documents.number (EID/visa) plaintext + readable by every workspace member", status: "missing", priority: "P1", effort: "M", notes: "Needs design: RLS tightening (HR-role only) + pgcrypto column encryption + API masking. UAE PDPL compliance call — Asad to decide retention + access policy." },
  ...
  { id: "scan-se-002-xlsx", category: "security", phase: "hardening", title: "[SE-002] xlsx@^0.18.5 unpatched (prototype pollution + ReDoS)", status: "missing", priority: "P1", effort: "M", notes: "Needs Asad decision: swap to exceljs (MIT, npm-published) OR pin cdn.sheetjs.com tarball in package.json. SheetJS deprecated npm releases — only patched versions are on their CDN." },
  ...
  { id: "scan-sb-008-prompt-injection", category: "security", phase: "hardening", title: "[SB-008] Indirect prompt injection: tool outputs into LLM context unsanitised", status: "missing", priority: "P2", effort: "L", notes: "OWASP LLM01. Needs design: untrusted-data delimiter + control-char strip + optional safety classifier on tool outputs. Deferred — bigger architecture pass." },
  ...
  { id: "scan-se-008-xfo-embed", category: "security", phase: "hardening", title: "[SE-008] Unconditional X-Frame-Options overrides /embed/* rule", status: "done", priority: "P2", effort: "XS", notes: "FIXED 2026-05-15 (V-5 + parent middleware patch): isEmbedPath() guard in applySecurityHeaders + middleware.ts now passes path to the helper. Skips XFO + frame-ancestors for /embed/* and /p|q|r|b|d/* toShare viewers." },

  // P3 — Low (3 done in same sweep; 4 deferred as low-impact hygiene)
  { id: "scan-sa-005-paddle-retention", category: "security", phase: "polish", title: "[SA-005] paddle_webhook_events.payload retains PII indefinitely", status: "missing", priority: "P3", effort: "S", notes: "Needs retention-policy decision (Asad call): daily cron to prune >90d processed rows, or strip PII at insert. Service-role-only access already in place." },
  { id: "scan-sa-007-invite-unconfirmed", category: "security", phase: "polish", title: "[SA-007] accept_workspace_invite doesn't verify email_confirmed_at", status: "missing", priority: "P3", effort: "XS", notes: "Defence in depth — Supabase default already blocks unconfirmed sessions. Add explicit check on a future migration pass." },
  { id: "scan-sb-009-persona-inject", category: "security", phase: "polish", title: "[SB-009] Workspace-admin persona_description injected into system prompt", status: "missing", priority: "P3", effort: "S", notes: "Workspace-admin → workspace-user, not cross-tenant. Strip newlines + role-tags + length-cap on a future pass." },
  { id: "scan-sb-010-admin-sql-fns", category: "security", phase: "polish", title: "[SB-010] Admin SQL console allows pg_sleep / pg_terminate_backend / pg_read_file", status: "missing", priority: "P3", effort: "S", notes: "Verify exec_admin_sql RPC runs as low-priv role + extend function blocklist. Admin-only path, low impact." },
