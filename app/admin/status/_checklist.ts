...
export const CHECKLIST: Item[] = [
  ...
  { id: "suspicious-login-alerts", category: "security", phase: "hardening", title: "Suspicious-login email (new device / location)", status: "partial", priority: "P2", effort: "M", notes: "Shipped 2026-05-17 (S5) + 2026-05-19 (Z3 wired): /api/cron/suspicious-login-scan now calls sendEmail() with the suspicious-login template; respects notification_prefs.email_suspicious_login. Sender falls through to email_outbox when no RESEND_API_KEY/POSTMARK_API_KEY env set — flip-the-switch when you sign up." },
  ...
  { id: "outbox-pattern", category: "reliability", phase: "scale", title: "Outbox pattern for cross-system writes", status: "done", priority: "P2", effort: "L", notes: "Shipped 2026-05-19 (Y1): event_outbox table (status/attempts/dedupe_key) + claim_outbox_batch FOR UPDATE SKIP LOCKED RPC + /api/cron/outbox-relay every 5 min. Producers emit via lib/outbox::emit (webhook.deliver / notification.fanout / audit.record + 5 reserved types)." },
  ...
  { id: "stuck-job-detection", category: "reliability", phase: "hardening", title: "Stuck-job detection + alert", status: "done", priority: "P1", effort: "S", notes: "Shipped 2026-05-18 (W1): /api/cron/stuck-jobs-detect every 5 min finds workflow_runs + ai_batch_jobs stuck in running >30 min, flips to 'stuck' status, emits in-app notifications to admins. Threshold tunable via ?minutes= param." },
  ...
  { id: "anomaly-detection", category: "observability", phase: "scale", title: "Anomaly detection on key metrics", status: "done", priority: "P3", effort: "L", notes: "Shipped 2026-05-19 (Y6 migration 20260519e): /api/cron/anomaly-check every 30 min compares api_latency_summary(30) p95 + err_rate to historical baseline (last 7 days same hour); alerts admins via anomaly_alert RPC when p95 >=3x baseline or err_rate >=5% with sample-size gates." },
