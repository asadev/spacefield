...
export const CHECKLIST: Item[] = [
  ...
  { id: "toshare", category: "product", phase: "foundation", title: "toShare universal sharing (toshare.net)", status: "done", priority: "P1", effort: "L" },
  ...
  { id: "ai-output-moderation", category: "ai", phase: "hardening", title: "Output moderation on AI content posted publicly", status: "partial", priority: "P1", effort: "M", notes: "Reputational risk if a toShare listing contains slurs/PII." },
  ...
  { id: "materialized-views", category: "database", phase: "scale", title: "Materialized views for heavy aggregations", status: "missing", priority: "P3", effort: "M", notes: "toShare analytics, insights dashboards." },
  ...
  // P0 — Critical
  { id: "scan-sb-001-quote-xss", category: "security", phase: "hardening", title: "[SB-001] Stored XSS in /q/[slug] via quote.terms (dangerouslySetInnerHTML, unescaped)", status: "missing", priority: "P0", effort: "S", notes: "app/solutions/tools/quote-builder/page.tsx:810-812 builds termsHtml from raw textarea; app/(share)/q/[slug]/page.tsx:122-125 renders via dangerouslySetInnerHTML. CSP is Report-Only so unmitigated." },
  { id: "scan-sb-002-embed-xss", category: "security", phase: "hardening", title: "[SB-002] Stored XSS in /p/[slug] via embed block.html", status: "missing", priority: "P0", effort: "S", notes: "app/(share)/p/[slug]/page.tsx:168-169 renders block.html raw. lib/toshare/server.ts:50-85 accepts arbitrary payload without sanitising. Drop embed kind or DOMPurify." },
  { id: "scan-sc-001-service-role-fallback", category: "security", phase: "hardening", title: "[SC-001] Service-role client silently falls back to anon key when env missing", status: "missing", priority: "P0", effort: "XS", notes: "lib/supabase/admin.ts:14-17 + mirror in middleware-helpers/runtime-banner/runtime-brand. Production trip-wire: missing SUPABASE_SERVICE_ROLE_KEY silently disables observability + audit + rate-limit writes. Throw on missing env." },
  { id: "scan-sd-001-contact-unauth", category: "security", phase: "hardening", title: "[SD-001] /api/contact: no auth, no rate-limit, sends 2 emails per call (mailbomb vector)", status: "missing", priority: "P0", effort: "XS", notes: "app/api/contact/route.ts:30-90. Rewrap with withApiHandler + rateLimit (5/60s by IP) + honeypot." },
  { id: "scan-sd-002-toshare-mint-rate", category: "security", phase: "hardening", title: "[SD-002] /api/toshare/mint: no rate-limit, no slug allowlist", status: "missing", priority: "P0", effort: "S", notes: "app/api/toshare/mint/route.ts:14-40. Add rate-limit (30/60s) + reserved-slug list (signin/admin/api/spacefield/etc)." },
  { id: "scan-se-001-next-cve", category: "security", phase: "hardening", title: "[SE-001] next@^16.2.4 has 7 advisories including 3 middleware-bypass CVEs", status: "missing", priority: "P0", effort: "XS", notes: "package.json:71. Codebase puts ALL security gating (IP rules, rate limits, maintenance, headers) in middleware — a bypass means none of it fires. Bump to ^16.2.6+." },

  // P1 — High
  { id: "scan-sa-001-toshare-mint-workspace", category: "security", phase: "hardening", title: "[SA-001] toshare_mint RPC trusts client workspace_id without membership check", status: "missing", priority: "P1", effort: "S", notes: "supabase/migrations/20260502_toshare_links.sql:64-121. RPC is security-definer + skips is_workspace_member(p_workspace_id). Squat on other tenants' subdomains." },
  { id: "scan-sa-002-toshare-anon-rpcs", category: "security", phase: "hardening", title: "[SA-002] toshare_record_submit/view granted to anon, no rate-limit or payload-size cap", status: "missing", priority: "P1", effort: "M", notes: "supabase/migrations/20260502_toshare_links.sql:160-181,230-231. Direct PostgREST /rpc/ hits bypass middleware rate-limit. Bloat toshare_events." },
  { id: "scan-sb-003-webhook-ssrf", category: "security", phase: "hardening", title: "[SB-003] SSRF via payload.webhookUrl: fetch(userInput) with response excerpt logged", status: "missing", priority: "P1", effort: "S", notes: "lib/toshare/webhook-sign.ts:133, lib/webhooks/sign.ts:108, lib/workflow-runner.ts:427. Block private/loopback/link-local IPs + non-https + cloud metadata." },
  { id: "scan-sb-004-help-md-xss", category: "security", phase: "hardening", title: "[SB-004] XSS via javascript:/data: URI scheme in admin help-article markdown links", status: "missing", priority: "P1", effort: "XS", notes: "app/admin/help/_markdown.ts:53-58 + _markdownClient.ts:34-38. Add scheme allowlist /^(https?:|mailto:|tel:|\\/|#)/i in inline()." },
  { id: "scan-sb-005-or-filter-injection", category: "security", phase: "hardening", title: "[SB-005] PostgREST .or()-filter injection in AI CRM search tools (no sanitisation)", status: "missing", priority: "P1", effort: "S", notes: "lib/agent/skills/crm-contacts/index.ts:54-56, crm-companies/index.ts:49. LLM-supplied query interpolated raw. Strip ,()%\\\\* and use multiple .ilike unions." },
  { id: "scan-sb-006-or-filter-weak", category: "security", phase: "hardening", title: "[SB-006] .or() filter has weaker escaping (no %\\ in tasks; no () in people)", status: "missing", priority: "P1", effort: "XS", notes: "lib/tasks/server.ts:88,467, lib/ai-tools/tasks.ts:221, lib/ai-tools/people.ts:71, lib/people/server.ts:72. Unify on lib/admin/search/page.tsx:442 escapeForLike() helper." },
  { id: "scan-sb-007-csv-formula-inj", category: "security", phase: "hardening", title: "[SB-007] CSV formula injection in admin exports (no =/@/+/-/\\t escape)", status: "missing", priority: "P1", effort: "XS", notes: "app/admin/database/_helpers.ts:280-285 csvCell + app/admin/people/export/route.ts:57-62. Prefix leading =/@/+/-/\\t with single quote ' per OWASP." },
  { id: "scan-sc-002-cron-secret-url", category: "security", phase: "hardening", title: "[SC-002] CRON_SECRET sent in URL query string, logged by Vercel", status: "missing", priority: "P1", effort: "XS", notes: "app/admin/jobs/_actions.ts:106. Move to Authorization: Bearer header; bearer fallback already exists in lib/cron/_check_enabled.ts." },
  { id: "scan-sc-003-error-leak", category: "security", phase: "hardening", title: "[SC-003] Raw DB error messages echoed to API clients across ~30 routes", status: "missing", priority: "P1", effort: "M", notes: "lib/api-wrap.ts:215-218 + 30+ route files return err.message. Add safeErrorMessage() that returns generic in prod, logs full via logError." },
  { id: "scan-sc-004-error-page", category: "security", phase: "hardening", title: "[SC-004] app/error.tsx + global-error.tsx render error.message in production", status: "missing", priority: "P1", effort: "XS", notes: "app/error.tsx:18, app/global-error.tsx:80. Show error.digest only; drop raw message JSX." },
  { id: "scan-sc-005-pii-eid-plaintext", category: "security", phase: "hardening", title: "[SC-005] employee_documents.number (Emirates ID/visa) plaintext + readable by every workspace member", status: "missing", priority: "P1", effort: "M", notes: "supabase/migrations/20260514e_people.sql:281-316. UAE PDPL classifies EID as sensitive. Tighten RLS (HR-role only) + pgcrypto column encryption + mask in API responses." },
  { id: "scan-sd-003-comments-rate", category: "security", phase: "hardening", title: "[SD-003] /api/comments POST/PATCH/DELETE: no rate-limit, no mention cap (notification fanout DoS)", status: "missing", priority: "P1", effort: "XS", notes: "app/api/comments/route.ts:64-160. Add rateLimit 30/60s/user + cap mentions per comment (max 10)." },
  { id: "scan-sd-004-people-mass-assign", category: "security", phase: "hardening", title: "[SD-004] /api/people/employees POST: no auth check at route boundary + mass-assignment (user_id from body)", status: "missing", priority: "P1", effort: "S", notes: "app/api/people/employees/route.ts:54-89. Add auth.getUser() 401. Zod schema. Reject user_id in POST body (only via invitation flow)." },
  { id: "scan-sd-005-people-routes", category: "security", phase: "hardening", title: "[SD-005] /api/people/onboarding|time-off|documents: same skip-auth + mass-assignment as SD-004", status: "missing", priority: "P1", effort: "S", notes: "Same pattern across 3 routes. Standardize on withApiHandler + Zod." },
  { id: "scan-sd-006-webhook-ssrf-admin", category: "security", phase: "hardening", title: "[SD-006] Admin outgoing webhook test: no SSRF allowlist on URL", status: "missing", priority: "P1", effort: "S", notes: "app/admin/webhooks/_actions.ts:286-350 + lib/webhooks/sign.ts:108. Admin (or compromised admin) can hit 169.254.169.254 metadata. Block private IPs at delivery." },
  { id: "scan-se-002-xlsx", category: "security", phase: "hardening", title: "[SE-002] xlsx@^0.18.5: prototype pollution + ReDoS, patched only on cdn.sheetjs.com", status: "missing", priority: "P1", effort: "M", notes: "package.json:82. Decision needed: swap to exceljs (MIT, npm) OR pin cdn.sheetjs.com tarball." },
  { id: "scan-se-003-csp-no-reporter", category: "security", phase: "hardening", title: "[SE-003] CSP Report-Only has no report-uri/report-to — violations silently discarded", status: "missing", priority: "P1", effort: "S", notes: "lib/security-headers.ts:98. Wire /api/security/csp-report → error_events table, then add report-uri/report-to directives so the policy buys us anything." },

  // P2 — Medium
  { id: "scan-sa-003-contact-rate", category: "security", phase: "hardening", title: "[SA-003] /api/contact rate-limit (dupe-coverage of SD-001 but logged separately)", status: "missing", priority: "P2", effort: "XS" },
  { id: "scan-sa-004-task-get-unauth", category: "security", phase: "hardening", title: "[SA-004] /api/tasks/:id + /api/projects/:id GET skip auth (rely on RLS only)", status: "missing", priority: "P2", effort: "XS", notes: "Asymmetric vs PATCH/DELETE; will regress if a future refactor uses admin client. Add 401 short-circuit." },
  { id: "scan-sb-008-prompt-injection", category: "security", phase: "hardening", title: "[SB-008] Indirect prompt injection: tool outputs concatenated into LLM context unsanitised", status: "missing", priority: "P2", effort: "L", notes: "lib/agent/runtime/executor.ts:253-259. OWASP LLM01. Wrap tool output with untrusted-data delimiter + safety pass + strip control chars." },
  { id: "scan-sb-011-file-url-xss", category: "security", phase: "hardening", title: "[SB-011] employee_documents.file_url not validated (javascript:/data: URI possible)", status: "missing", priority: "P2", effort: "XS", notes: "lib/people/actions.ts:286-322. Validate against /^https?:\\/\\//i at insert; use safeHref() at render." },
  { id: "scan-sc-006-inbound-token-url", category: "security", phase: "hardening", title: "[SC-006] Inbound webhook ?token= fallback ends up in Vercel logs", status: "missing", priority: "P2", effort: "S", notes: "app/api/inbound/webhook/[slug]/route.ts:8,61-72. Deprecate query-string auth; require HMAC sig for new sources." },
  { id: "scan-sc-007-waitlist-email-log", category: "security", phase: "hardening", title: "[SC-007] /waitlist action logs raw user emails in plaintext", status: "missing", priority: "P2", effort: "XS", notes: "app/waitlist/_actions.ts:63-81. Hash email before logging (sha256 prefix)." },
  { id: "scan-sd-007-health-leak", category: "security", phase: "hardening", title: "[SD-007] /api/health publicly exposes commit SHA + region + probe error details", status: "missing", priority: "P2", effort: "XS", notes: "app/api/health/route.ts:60-79. Drop commit from public body (let attackers pinpoint vulnerable releases); move details to ?deep=1 + token." },
  { id: "scan-sd-008-cron-timing", category: "security", phase: "hardening", title: "[SD-008] cron-secret comparison is === not crypto.timingSafeEqual", status: "missing", priority: "P2", effort: "XS", notes: "app/api/cron/social-publish/route.ts. Use timingSafeEqual on equal-length buffers." },
  { id: "scan-sd-009-savecontent-type", category: "security", phase: "hardening", title: "[SD-009] /api/files/save-content accepts contentType from body with no allowlist", status: "missing", priority: "P2", effort: "XS", notes: "app/api/files/save-content/route.ts:175-176. Maintain content-type allowlist (json/text/markdown/sheets)." },
  { id: "scan-sd-013-no-body-limit", category: "security", phase: "hardening", title: "[SD-013] No app-level bodySizeLimit; 100MB JSON POSTs fully buffer", status: "missing", priority: "P2", effort: "XS", notes: "next.config.ts missing experimental.serverActions.bodySizeLimit. Cap at 2mb. Add per-route content-length check in withApiHandler." },
  { id: "scan-se-004-csp-unsafe-eval", category: "security", phase: "hardening", title: "[SE-004] CSP allows 'unsafe-eval' in script-src (no audit done)", status: "missing", priority: "P2", effort: "S", notes: "lib/security-headers.ts:61. Modern Next/React/Tailwind/Framer don't need eval. Drop and observe via report-uri after SE-003." },
  { id: "scan-se-005-protobufjs", category: "security", phase: "hardening", title: "[SE-005] protobufjs <7.5.6 transitive (via Univer): code-injection + proto-pollution + DoS", status: "missing", priority: "P2", effort: "XS", notes: "Reachable via @univerjs/protocol→@grpc/grpc-js→@grpc/proto-loader→protobufjs. Add pnpm.overrides entry pinning protobufjs@>=7.5.6." },
  { id: "scan-se-008-xfo-embed", category: "security", phase: "hardening", title: "[SE-008] Unconditional X-Frame-Options: SAMEORIGIN overrides /embed/* ALLOWALL rule", status: "missing", priority: "P2", effort: "XS", notes: "lib/security-headers.ts:94 conflicts with next.config.ts:155-156. Add isEmbedPath() guard in applySecurityHeaders." },

  // P3 — Low (curated subset; full list in scanner reports stored as memory)
  { id: "scan-sa-005-paddle-retention", category: "security", phase: "polish", title: "[SA-005] paddle_webhook_events.payload retains PII (customer emails) indefinitely", status: "missing", priority: "P3", effort: "S", notes: "supabase/migrations/20260428_paddle_billing.sql:13-19. Daily cron to prune processed >90d; or strip PII at insert." },
  { id: "scan-sa-007-invite-unconfirmed", category: "security", phase: "polish", title: "[SA-007] accept_workspace_invite doesn't verify email_confirmed_at", status: "missing", priority: "P3", effort: "XS", notes: "supabase/migrations/20260427_workspace_sharing.sql:347-352. Defence in depth — Supabase default blocks unconfirmed sessions; verify explicitly." },
  { id: "scan-sb-009-persona-inject", category: "security", phase: "polish", title: "[SB-009] Workspace-admin persona_description injected into system prompt", status: "missing", priority: "P3", effort: "S", notes: "lib/agent/runtime/persona.ts:87-95. Strip newlines + role-tags; length-cap; gate writes behind admin role." },
  { id: "scan-sb-010-admin-sql-fns", category: "security", phase: "polish", title: "[SB-010] Admin SQL console allows pg_sleep / pg_terminate_backend / pg_read_file", status: "missing", priority: "P3", effort: "S", notes: "lib/admin/sql-console.ts:60-65. Verify exec_admin_sql RPC runs as low-priv role + extend function blocklist." },
  { id: "scan-sc-010-help-cookie", category: "security", phase: "polish", title: "[SC-010] /api/admin/help/vote anon-id cookie missing Secure flag", status: "missing", priority: "P3", effort: "XS", notes: "Add secure: process.env.NODE_ENV === 'production'." },
  { id: "scan-sc-013-bak-file", category: "security", phase: "polish", title: "[SC-013] public/logos/wasl.png.bak stray file in repo", status: "missing", priority: "P3", effort: "XS", notes: "Just rm it." },
  { id: "scan-se-012-pre-push-npm", category: "security", phase: "polish", title: "[SE-012] .githooks/pre-push uses 'npm run build' (should be pnpm)", status: "missing", priority: "P3", effort: "XS", notes: "Inconsistent with pnpm-lock.yaml; fresh-checkout double-installs." },
