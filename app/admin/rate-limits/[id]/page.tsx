...
export default async function EditRateLimitRulePage({ params }: PageProps) {
  ...
  const rule = (data as RateLimitRuleRow | null) ?? null;
  if (!rule) notFound();
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <h1 className="mt-2 text-xl font-semibold text-app">Edit rate-limit rule</h1>
        <p className="mt-0.5 font-mono text-[11px] text-muted">{rule.id}</p>
      </div>

      <RuleForm rule={rule} action={updateRule} submitLabel="Save changes" />
