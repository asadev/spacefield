...
export async function updateWorkspaceBranding(
  ...
  const colorRaw = String(formData.get("toshare_brand_color") ?? "").trim();

  const slug = slugRaw.length ? slugRaw : null;
  const avatar_url = avatarRaw.length ? avatarRaw : null;
  let toshare_brand_color: string | null = null;
  if (colorRaw.length) {
    if (!/^#[0-9a-fA-F]{6}$/.test(colorRaw)) {
      throw new Error("toshare_brand_color must be #RRGGBB");
    }
    toshare_brand_color = colorRaw;
  }

  const admin = createAdminClient();
  const { data: before } = await admin
    .from("workspaces")
    .select("id, name, slug, description, avatar_url, toshare_brand_color")
    .eq("id", workspaceId)
    .maybeSingle();
  ...
  patch.toshare_brand_color = toshare_brand_color;
  ...
  await recordAdminAction({
    ...
    before: {
      ...
      toshare_brand_color: before.toshare_brand_color,
    },
    after: {
