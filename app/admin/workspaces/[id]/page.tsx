...
export default async function AdminWorkspaceDetailPage({
  ...
  const wsRes = await admin
    .from("workspaces")
    .select(
      "id, user_id, name, slug, description, avatar_url, share_brand_color, share_subdomain, archived_at, created_at, updated_at"
    )
    .eq("id", id)
    .maybeSingle();
    ...
    <div className="space-y-5">
      ...
      <section className="flex flex-wrap items-start gap-4 rounded-xl border border-app bg-app-elevated p-5">
        <div
          ...
          style={{
            backgroundColor:
              workspace.share_brand_color ?? "var(--bg-app)",
            color: workspace.share_brand_color ? "#fff" : undefined,
          }}
        >
  ...
  return (
    <>
      <SectionCard
        ...
        hint="Identity + branding. Slug is also the Share link prefix."
      >
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_220px]">
            ...
            <label className="block text-xs">
              <span className="mb-1 block text-faint">
                Share brand color (#RRGGBB)
              </span>
              <input
                name="share_brand_color"
                defaultValue={workspace.share_brand_color ?? ""}
                placeholder="#0ea5e9"
                pattern="#[0-9a-fA-F]{6}"
      ...
      >
        ...
        <div className="mt-3 grid gap-1 text-[11px] text-muted">
          <div>
            share subdomain:{" "}
            {workspace.share_subdomain ? (
              <code className="text-secondary">{workspace.share_subdomain}.share.example.com</code>
            ) : (
