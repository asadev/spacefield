...
export default async function AdminAnalyticsPage({
  ...
  const [
    ...
    userClient.rpc("share_global_analytics", { p_since: since }),
  ...
  const events = (eventsRecentRes.data ?? []) as shareEventRow[];
  const links = (linksRes.data ?? []) as shareLinkRow[];
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted">
            Cross-workspace view of every Share link, event, and webhook
            delivery. Read-only.
          </p>
        </div>
      ...
      <section className="grid gap-3 md:grid-cols-[minmax(0,1fr)_280px]">
        ...
        <div className="rounded-xl border border-app bg-app-elevated p-4">
          ...
          <div className="mt-3 text-[11px] text-muted">
            ...
            <code className="font-mono text-secondary">share_webhook_deliveries</code>.
          </div>
        </div>
      </section>
