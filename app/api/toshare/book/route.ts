...
export async function POST(req: NextRequest) {
  try {
    ...
    const supabase = await createClient();
    const { error: rpcErr } = await supabase.rpc("toshare_record_booking", {
      p_link_id: link.id,
    ...
    notifyBooking({ link, body }).catch((err) => {
      console.warn("[toshare/book] notify failed:", err);
    });
...
async function notifyBooking(args: { link: ToShareLinkRow; body: Record<string, unknown> }) {
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:24px;">
    ...
    <div style="color:#334155;font-size:14px;line-height:1.6;">
      ...
      console.warn("[toshare/book] host email failed:", err);
    }
  }
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:24px;">
    ...
    console.warn("[toshare/book] invitee email failed:", err);
  }
}
