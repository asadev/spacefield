 ...
 *
 * Records a `convert` event in toshare_events with the acceptance details
...
import type { QuotePayload, ToShareLinkRow } from "@/lib/toshare/types";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const supabase = await createClient();
    const { error: evtErr } = await supabase.rpc("toshare_record_accept", {
      p_link_id: link.id,
    ...
    notifyAccept({ link, body }).catch((err) => {
      console.warn("[toshare/accept] notify failed:", err);
    });
...
async function notifyAccept(args: { link: ToShareLinkRow; body: Record<string, unknown> }) {
  const payload = args.link.payload as unknown as QuotePayload;
  const url = buildToShareUrl(args.link);
  ...
  if (payload.webhookUrl) {
    try {
      ...
      console.warn("[toshare/accept] webhook failed:", err instanceof Error ? err.message : err);
    }
  }
  ...
  <div style="text-align:center;margin-top:14px;font-size:11px;color:#94a3b8;">Sent via toshare.net</div>
</body></html>`;
    try {
      await sendEmail({
      ...
      console.warn("[toshare/accept] email failed:", err instanceof Error ? err.message : err);
    }
  }
}
