...
- **Embed CSP carve-out** — `lib/security-headers.ts:103-111` `isEmbedPath()` correctly matches `/embed/*` and Share viewer prefixes; drops `X-Frame-Options` + `frame-ancestors` for those responses. **SE-008 confirmed for the embed namespace.**
