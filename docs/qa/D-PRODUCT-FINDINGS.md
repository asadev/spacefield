...
Saved views, Import wizard, Share viewers, CurrencySwitcher.
...
| Share `/p|/q|/r|/b|/d/[slug]` viewers each read `?ws=` and pass `subdomain` to `resolveLink` | `app/(share)/p/[slug]/page.tsx:18-21`, etc. + `lib/share/server.ts:136-147` |
| Share middleware host router rewrites `<ws>.share.example.com/<type>/<slug>` → adds `?ws=<sub>` | `middleware.ts:188-245` |
| Share share-only-paths guard (404 on spacefield.io/.co) | `middleware.ts:204-209` |
...
| `recordView` fires on every Share viewer page (`p`, `q`, `r`, `b`, `d`, `share-form`) | `app/(share)/*/[slug]/page.tsx` |
...
- **Files**: `app/tasks/[id]/page.tsx`, `app/projects/[id]/page.tsx`, `app/people/[id]/page.tsx` — none of them call `recordView` from `lib/recents`. The only call sites are the Share viewers and the Cmd-K palette itself (`grep -rln "recordView"` yields 8 files, all in `(share)` or `CommandPalette.tsx`).
