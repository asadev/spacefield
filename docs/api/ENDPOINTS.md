...
- **signed** — provider HMAC / signature header (Paddle, Meta, toShare).
...
| Method | Path | Auth | Rate-limit | Purpose |
|--------|------|------|------------|---------|
| POST | /api/toshare/mint | session | — | Mint a new shareable link from any tool output. |
| GET, POST | /api/toshare/links | session | — | List/manage workspace links. |
| GET, PATCH, DELETE | /api/toshare/links/[id] | session | — | One link. |
| POST | /api/toshare/links/[id]/pause | session | — | Pause a link (returns 404 to visitors). |
| POST | /api/toshare/links/[id]/resume | session | — | Unpause a link. |
| GET | /api/toshare/links/[id]/deliveries | session | — | Webhook delivery log for a link. |
| POST | /api/toshare/upload | session | — | Upload private file to toshare-files bucket. |
| POST | /api/toshare/upload-image | session | — | Upload public image to toshare-public bucket. |
| GET | /api/toshare/download/[id] | none | — | Signed download for a shared file (link-token gated). |
| POST | /api/toshare/submit | none | per-IP submit | Public form submission against a minted link. |
| POST | /api/toshare/accept | none | — | Quote accept event. |
| POST | /api/toshare/book | none | per-IP book | Booking submission. |
| POST | /api/toshare/subdomain | session | — | Claim/release a workspace's `<sub>.toshare.net`. |
