...
**only Asad has Studio access; everyone else operates via the
...
| ------------------ | ------------------------------------ | ------------------------------------------------------- |
| **Owner**          | Asad                                 | Supabase Studio (full DDL + DML), service-role key      |
...
There is no per-engineer Supabase login other than Asad's. When we hire
...
Asad and have them use the application's admin panel. We default to
...
2. Asad sends a project invite from Supabase Dashboard → Team.
...
5. On revocation, Asad removes from Supabase Team AND from Vercel
...
- **`psql` via Supabase connection string**: the connection string is
  ...
  Asad's MacBook can read this file — that's why the laptop is the
  bottleneck.
...
schema-level changes, recovering a corrupted row, etc.), Asad uses
